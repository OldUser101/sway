...
{
    struct sway_container *con = wl_container_of(listener, con, output_enter);
    struct wlr_scene_output *output = data;

    if (con->view->foreign_toplevel) {
        wlr_foreign_toplevel_handle_v1_output_enter(con->view->foreign_toplevel,
                                                    output->output);
    }
}
...
{
    struct sway_container *con = wl_container_of(listener, con, output_leave);
    struct wlr_scene_output *output = data;

    if (con->view->foreign_toplevel) {
        wlr_foreign_toplevel_handle_v1_output_leave(con->view->foreign_toplevel,
                                                    output->output);
    }
}
...
handle_destroy(struct wl_listener *listener, void *data)
{
    struct sway_container *con =
        wl_container_of(listener, con, output_handler_destroy);

    container_begin_destroy(con);
}

static bool
handle_point_accepts_input(struct wlr_scene_buffer *buffer, double *x,
                           double *y)
{
    return false;
}
...
alloc_rect_node(struct wlr_scene_tree *parent, bool *failed)
{
    if (*failed) {
        return NULL;
    }
    ...
    struct wlr_scene_rect *rect =
        wlr_scene_rect_create(parent, 0, 0, (float[4]){0.f, 0.f, 0.f, 1.f});
    if (!rect) {
...
container_create(struct sway_view *view)
{
    ...
    //       border everything gets disabled. We only render the content iff
    //       there is a border as well)
...
static bool
container_is_focused(struct sway_container *con, void *data)
{
    return con->current.focused;
}

static bool
container_has_focused_child(struct sway_container *con)
{
    return container_find_child(con, container_is_focused, NULL);
}

static bool
container_is_current_parent_focused(struct sway_container *con)
{
    if (con->current.parent) {
        struct sway_container *parent = con->current.parent;
        return parent->current.focused ||
               container_is_current_parent_focused(parent);
...
container_get_current_colors(struct sway_container *con)
{
    struct border_colors *colors;

    bool urgent =
        con->view ? view_is_urgent(con->view) : container_has_urgent_child(con);
    ...
    if (urgent) {
        colors = &config->border_colors.urgent;
               ...
               container_is_current_parent_focused(con)) {
        colors = &config->border_colors.focused;
    } else if (config->has_focused_tab_title &&
               container_has_focused_child(con)) {
...
static bool
container_is_current_floating(struct sway_container *container)
{
...
static void
scene_rect_set_color(struct wlr_scene_rect *rect, const float color[4],
                     float opacity)
{
...
void
container_update(struct sway_container *con)
{
    ...
    if (!container_is_current_floating(con) && siblings &&
        siblings->length == 1) {
        if (layout == L_HORIZ) {
    ...
    if (con->title_bar.title_text) {
        ...
        sway_text_node_set_background(con->title_bar.title_text,
                                      colors->background);
    }
...
void
container_update_itself_and_parents(struct sway_container *con)
{
...
static void
update_rect_list(struct wlr_scene_tree *tree, pixman_region32_t *region)
{
    int len;
...
void
container_arrange_title_bar(struct sway_container *con)
{
    ...
    if (con->title_bar.marks_text) {
        ...
        wlr_scene_node_set_position(node->node, h_padding,
                                    (height - node->height) >> 1);

        pixman_region32_union_rect(&text_area, &text_area, node->node->x,
                                   node->node->y, alloc_width, node->height);
    }
...
void
container_update_marks(struct sway_container *con)
{
    char *buffer = NULL;
    ...
    if (!buffer) {
        ...
        con->title_bar.marks_text = sway_text_node_create(
            con->title_bar.tree, buffer, colors->text, false);
    } else {
...
void
container_update_title_bar(struct sway_container *con)
{
    ...
    con->title_bar.title_text =
        sway_text_node_create(con->title_bar.tree, con->formatted_title,
                              colors->text, config->pango_markup);
...
void
container_destroy(struct sway_container *con)
{
    if (!sway_assert(
            con->node.destroying,
    ...
    if (!sway_assert(con->node.ntxnrefs == 0,
                     "Tried to free container "
...
void
container_begin_destroy(struct sway_container *con)
{
    if (con->view) {
...
void
container_reap_empty(struct sway_container *con)
{
    if (con->view) {
        return;
    }
...
struct sway_container *
container_flatten(struct sway_container *container)
{
    if (container->view) {
        return NULL;
    }
...
struct sway_container *
container_find_child(struct sway_container *container,
...
void
container_for_each_child(struct sway_container *container,
                         void (*f)(struct sway_container *container,
                                   void *data),
                         void *data)
{
    if (container->pending.children) {
        for (int i = 0; i < container->pending.children->length; ++i) {
            struct sway_container *child =
                container->pending.children->items[i];
            f(child, data);
...
struct sway_container *
container_obstructing_fullscreen_container(struct sway_container *container)
{
...
bool
container_has_ancestor(struct sway_container *descendant,
 ...
 */
size_t
parse_title_format(struct sway_container *container, char *buffer)
{
    if (!container->title_format ||
        strcmp(container->title_format, "%title") == 0) {
        if (container->view) {
            return append_prop(buffer, view_get_title(container->view));
        } else {
            return container_build_representation(
                container->pending.layout, container->pending.children, buffer);
        }
    }
    ...
    while (next) {
        ...
        if (has_prefix(next, "%title")) {
            if (container->view) {
                ...
                len += container_build_representation(
                    container->pending.layout, container->pending.children,
                    buffer);
            }
            format += strlen("%title");
            ...
            if (has_prefix(next, "%app_id")) {
                ...
                len += append_prop(buffer,
                                   view_get_sandbox_engine(container->view));
                format += strlen("%sandbox_engine");
            } else if (has_prefix(next, "%sandbox_app_id")) {
                len += append_prop(buffer,
                                   view_get_sandbox_app_id(container->view));
                format += strlen("%sandbox_app_id");
            } else if (has_prefix(next, "%sandbox_instance_id")) {
                len += append_prop(
                    buffer, view_get_sandbox_instance_id(container->view));
 ...
 */
size_t
container_build_representation(enum sway_container_layout layout,
...
void
container_update_representation(struct sway_container *con)
{
    if (!con->view) {
...
size_t
container_titlebar_height(void)
{
    return config->font_height + config->titlebar_v_padding * 2;
}

void
floating_calculate_constraints(int *min_width, int *max_width, int *min_height,
                               int *max_height)
{
...
void
floating_fix_coordinates(struct sway_container *con, struct wlr_box *old,
                         struct wlr_box *new)
{
    if (!old->width || !old->height) {
        ...
        con->pending.x = new->x + (rel_x * new->width) / old->width -
                         (con->pending.width / 2);
        con->pending.y = new->y + (rel_y * new->height) / old->height -
                         (con->pending.height / 2);

        sway_log(SWAY_DEBUG, "Transformed container %p to coords (%f, %f)", con,
                 con->pending.x, con->pending.y);
    }
}

static void
floating_natural_resize(struct sway_container *con)
{
    ...
    if (!con->view) {
        con->pending.width =
            fmax(min_width, fmin(con->pending.width, max_width));
        con->pending.height =
            fmax(min_height, fmin(con->pending.height, max_height));
    } else {
...
void
container_floating_resize_and_center(struct sway_container *con)
{
    struct sway_workspace *ws = con->pending.workspace;
    if (!ws) {
    ...
    if (!con->view) {
        ...
        if (con->pending.content_width > ws->width ||
            ...
            con->pending.content_x =
                ob.x + (ob.width - con->pending.content_width) / 2;
            con->pending.content_y =
                ob.y + (ob.height - con->pending.content_height) / 2;
        } else {
            con->pending.content_x =
                ws->x + (ws->width - con->pending.content_width) / 2;
            con->pending.content_y =
                ws->y + (ws->height - con->pending.content_height) / 2;
        }
...
void
container_floating_set_default_size(struct sway_container *con)
{
    if (!sway_assert(con->pending.workspace,
                     "Expected a container on a workspace")) {
        return;
    }
 ...
 */
void
container_set_resizing(struct sway_container *con, bool resizing)
{
    if (!con) {
        return;
    }
...
void
container_set_floating(struct sway_container *container, bool enable)
{
...
void
container_set_geometry_from_content(struct sway_container *con)
{
    ...
    if (con->pending.border != B_CSD && !con->pending.fullscreen_mode) {
        border_width =
            con->pending.border_thickness * (con->pending.border != B_NONE);
        top = con->pending.border == B_NORMAL ? container_titlebar_height()
                                              : border_width;
    }
...
bool
container_is_floating(struct sway_container *container)
{
...
void
container_get_box(struct sway_container *container, struct wlr_box *box)
{
 ...
 */
void
container_floating_translate(struct sway_container *con, double x_amount,
                             double y_amount)
{
 ...
 */
struct sway_output *
container_floating_find_output(struct sway_container *con)
{
    ...
    for (int i = 0; i < root->outputs->length; ++i) {
        ...
        wlr_box_closest_point(&output_box, center_x, center_y, &closest_x,
                              &closest_y);
...
void
container_floating_move_to(struct sway_container *con, double lx, double ly)
{
...
void
container_floating_move_to_center(struct sway_container *con)
{
    if (!sway_assert(container_is_floating(con),
                     "Expected a floating container")) {
        return;
    ...
    container_floating_translate(con, new_lx - con->pending.x,
                                 new_ly - con->pending.y);
}
...
bool
container_has_urgent_child(struct sway_container *container)
{
    return container_find_child(container, find_urgent_iterator, NULL);
}

void
container_end_mouse_operation(struct sway_container *container)
{
...
static void
set_fullscreen(struct sway_container *con, bool enable)
{
    if (!con->view) {
        return;
    }
...
static void
container_fullscreen_workspace(struct sway_container *con)
{
...
static void
container_fullscreen_global(struct sway_container *con)
{
    if (!sway_assert(con->pending.fullscreen_mode == FULLSCREEN_NONE,
                     "Expected a non-fullscreen container")) {
        return;
...
void
container_fullscreen_disable(struct sway_container *con)
{
    if (!sway_assert(con->pending.fullscreen_mode != FULLSCREEN_NONE,
    ...
    // size and location
    if (container_is_floating(con) &&
        (con->pending.width == 0 || con->pending.height == 0)) {
...
void
container_set_fullscreen(struct sway_container *con,
        ...
        case FULLSCREEN_WORKSPACE:
            ...
            if (con->pending.workspace && con->pending.workspace->fullscreen) {
                container_fullscreen_disable(
                    con->pending.workspace->fullscreen);
            }
...
struct sway_container *
container_toplevel_ancestor(struct sway_container *container)
{
...
bool
container_is_floating_or_child(struct sway_container *container)
{
    return container_is_floating(container_toplevel_ancestor(container));
}

bool
container_is_fullscreen_or_child(struct sway_container *container)
{
    do {
...
list_t *
container_get_siblings(struct sway_container *container)
{
...
int
container_sibling_index(struct sway_container *child)
{
    return list_find(container_get_siblings(child), child);
}

void
container_handle_fullscreen_reparent(struct sway_container *con)
{
    if (con->pending.fullscreen_mode != FULLSCREEN_WORKSPACE ||
        !con->pending.workspace || con->pending.workspace->fullscreen == con) {
        return;
    }
...
void
container_insert_child(struct sway_container *parent,
...
void
container_add_sibling(struct sway_container *fixed,
...
void
container_add_child(struct sway_container *parent, struct sway_container *child)
{
    if (child->pending.workspace) {
        container_detach(child);
    }
...
void
container_detach(struct sway_container *child)
{
...
void
container_replace(struct sway_container *container,
...
struct sway_container *
container_split(struct sway_container *child, enum sway_container_layout layout)
{
...
bool
container_is_transient_for(struct sway_container *child,
                           struct sway_container *ancestor)
{
...
static bool
find_by_mark_iterator(struct sway_container *con, void *data)
{
    char *mark = data;
...
struct sway_container *
container_find_mark(char *mark)
{
    return root_find_container(find_by_mark_iterator, mark);
}

bool
container_find_and_unmark(char *mark)
{
    struct sway_container *con =
        root_find_container(find_by_mark_iterator, mark);
    if (!con) {
        return false;
    }
...
void
container_clear_marks(struct sway_container *con)
{
    for (int i = 0; i < con->marks->length; ++i) {
...
bool
container_has_mark(struct sway_container *con, char *mark)
{
    for (int i = 0; i < con->marks->length; ++i) {
...
void
container_add_mark(struct sway_container *con, char *mark)
{
...
void
container_raise_floating(struct sway_container *con)
{
...
bool
container_is_scratchpad_hidden(struct sway_container *con)
{
    return con->scratchpad && !con->pending.workspace;
}

bool
container_is_scratchpad_hidden_or_child(struct sway_container *con)
{
...
bool
container_is_sticky(struct sway_container *con)
{
    return con->is_sticky && container_is_floating(con);
}

bool
container_is_sticky_or_child(struct sway_container *con)
{
    return container_is_sticky(container_toplevel_ancestor(con));
}

static bool
is_parallel(enum sway_container_layout first, enum sway_container_layout second)
{
    switch (first) {
        case L_TABBED:
        case L_HORIZ:
...
static bool
container_is_squashable(struct sway_container *con,
                        struct sway_container *child)
{
    ...
    return (con->pending.layout == L_HORIZ || con->pending.layout == L_VERT) &&
           (child->pending.layout == L_HORIZ ||
            child->pending.layout == L_VERT) &&
           !is_parallel(con->pending.layout, child->pending.layout) &&
           is_parallel(gp_layout, child->pending.layout);
}

static void
container_squash_children(struct sway_container *con)
{
    for (int i = 0; i < con->pending.children->length; i++) {
        struct sway_container *child = con->pending.children->items[i];
...
int
container_squash(struct sway_container *con)
{
    if (!con->pending.children) {
        return 0;
    }
    ...
    if (container_is_squashable(con, child)) {
        ...
        while (child->pending.children->length) {
            ...
            if (con->pending.parent) {
                ...
                workspace_insert_tiling_direct(con->pending.workspace, current,
                                               idx);
            }
            change++;
        }
...
static void
swap_places(struct sway_container *con1, struct sway_container *con2)
{
...
static void
swap_focus(struct sway_container *con1, struct sway_container *con2,
           struct sway_seat *seat, struct sway_container *focus)
{
    if (focus == con1 || focus == con2) {
        ...
        if (focus == con1 && (layout2 == L_TABBED || layout2 == L_STACKED)) {
            ...
            seat_set_focus_container(seat, ws1 != ws2 ? con2 : con1);
                   ...
                   (layout1 == L_TABBED || layout1 == L_STACKED)) {
    ...
    if (root->fullscreen_global) {
        seat_set_focus(seat, seat_get_focus_inactive(
                                 seat, &root->fullscreen_global->node));
    }
}

void
container_swap(struct sway_container *con1, struct sway_container *con2)
{
    ...
    sway_log(SWAY_DEBUG, "Swapping containers %zu and %zu", con1->node.id,
             con2->node.id);
    ...
    int stored_prev_num = -1;
    if (seat->prev_workspace > -1) {
        stored_prev_num = seat->prev_workspace;
    }
    ...
    if (stored_prev_num > -1) {
        seat->prev_workspace = stored_prev_num;
    }
