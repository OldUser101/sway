...
bool
view_init(struct sway_view *view, enum sway_view_type type,
...
void
view_destroy(struct sway_view *view)
{
...
void
view_begin_destroy(struct sway_view *view)
{
...
const char *
view_get_title(struct sway_view *view)
{
...
const char *
view_get_app_id(struct sway_view *view)
{
    if (view->impl->get_string_prop) {
...
const char *
view_get_class(struct sway_view *view)
{
    if (view->impl->get_string_prop) {
...
const char *
view_get_instance(struct sway_view *view)
{
    if (view->impl->get_string_prop) {
...
#if WLR_HAS_XWAYLAND
uint32_t
view_get_x11_window_id(struct sway_view *view)
{
...
uint32_t
view_get_x11_parent_id(struct sway_view *view)
{
    if (view->impl->get_int_prop) {
...
#endif
const char *
view_get_window_role(struct sway_view *view)
{
    if (view->impl->get_string_prop) {
...
uint32_t
view_get_window_type(struct sway_view *view)
{
    if (view->impl->get_int_prop) {
...
security_context_from_view(struct sway_view *view)
{
...
const char *
view_get_sandbox_engine(struct sway_view *view)
{
    const struct wlr_security_context_v1_state *security_context =
...
const char *
view_get_sandbox_app_id(struct sway_view *view)
{
    const struct wlr_security_context_v1_state *security_context =
        security_context_from_view(view);
...
const char *
view_get_sandbox_instance_id(struct sway_view *view)
{
    const struct wlr_security_context_v1_state *security_context =
        security_context_from_view(view);
...
const char *
view_get_shell(struct sway_view *view)
{
    switch (view->type) {
...
void
view_get_constraints(struct sway_view *view, double *min_width,
                     double *max_width, double *min_height, double *max_height)
{
    if (view->impl->get_constraints) {
        view->impl->get_constraints(view, min_width, max_width, min_height,
                                    max_height);
    } else {
        *min_width = 1;
        *max_width = DBL_MAX;
...
uint32_t
view_configure(struct sway_view *view, double lx, double ly, int width,
               int height)
{
...
bool
view_inhibit_idle(struct sway_view *view)
{
...
bool
view_ancestor_is_only_visible(struct sway_view *view)
{
...
static bool
view_is_only_visible(struct sway_view *view)
{
    struct sway_container *con = view->container;
    while (con) {
        enum sway_container_layout layout = container_parent_layout(con);
...
static bool
gaps_to_edge(struct sway_view *view)
{
...
void
view_autoconfigure(struct sway_view *view)
{
    struct sway_container *con = view->container;
    ...
    if (!container_is_floating(con)) {
        ...
        // title area. We have to offset the surface y by the height of the
        // title, bar, and disable any top border because we'll always have the
        // title bar.
        list_t *siblings = container_get_siblings(con);
        bool show_titlebar =
    ...
    switch (con->pending.border) {
        ...
        case B_NORMAL:
            ...
            if (y_offset) {
                y = con->pending.y + y_offset;
                height =
                    con->pending.height - y_offset -
                    con->pending.border_thickness * con->pending.border_bottom;
            } else {
                y = con->pending.y + container_titlebar_height();
                height =
                    con->pending.height - container_titlebar_height() -
                    con->pending.border_thickness * con->pending.border_bottom;
            }
            break;
    }
...
void
view_set_activated(struct sway_view *view, bool activated)
{
...
void
view_request_activate(struct sway_view *view, struct sway_seat *seat)
{
...
void
view_request_urgent(struct sway_view *view)
{
...
void
view_set_csd_from_server(struct sway_view *view, bool enabled)
{
...
void
view_update_csd_from_client(struct sway_view *view, bool enabled)
{
...
void
view_set_tiled(struct sway_view *view, bool tiled)
{
...
void
view_close(struct sway_view *view)
{
    if (view->impl->close) {
...
void
view_close_popups(struct sway_view *view)
{
...
static bool
view_has_executed_criteria(struct sway_view *view, struct criteria *criteria)
{
...
void
view_execute_criteria(struct sway_view *view)
{
    ...
    for (int i = 0; i < criterias->length; i++) {
        ...
        list_t *res_list =
            execute_command(criteria->cmdlist, NULL, view->container);
        while (res_list->length) {
            struct cmd_results *res = res_list->items[0];
            if (res->status != CMD_SUCCESS) {
                sway_log(SWAY_ERROR, "for_window '%s' failed: %s",
                         criteria->raw, res->error);
            }
...
static void
view_populate_pid(struct sway_view *view)
{
    pid_t pid;
    switch (view->type) {
#if WLR_HAS_XWAYLAND
...
void
view_assign_ctx(struct sway_view *view, struct launcher_ctx *ctx)
{
    if (view->ctx) {
...
select_workspace(struct sway_view *view)
{
    struct sway_seat *seat = input_manager_current_seat();

    // Check if there's any `assign` criteria for the view
    list_t *criterias =
        criteria_for_view(view, CT_ASSIGN_WORKSPACE | CT_ASSIGN_OUTPUT);
    ...
    for (int i = 0; i < criterias->length; ++i) {
        ...
        if (criteria->type == CT_ASSIGN_OUTPUT) {
            ...
            ws = workspace_by_desc(criteria->target);
            ...
            if (!ws) {
                if (strcasecmp(criteria->target, "back_and_forth") == 0) {
                    ...
                    int ws_num = workspace_read_num(criteria->target);
                    sway_assert(ws_num != -1, "Workspace number cannot be -1");
...
static void
update_ext_foreign_toplevel(struct sway_view *view)
{
...
static bool
should_focus(struct sway_view *view)
{
    struct sway_seat *seat = input_manager_current_seat();
    ...
    if (!view->container->pending.parent && !prev_con) {
        size_t num_children =
            view->container->pending.workspace->tiling->length +
...
handle_foreign_fullscreen_request(struct wl_listener *listener, void *data)
{
    ...
    container_set_fullscreen(container, event->fullscreen ? FULLSCREEN_WORKSPACE
                                                          : FULLSCREEN_NONE);
    if (event->fullscreen) {
        arrange_root();
    } else {
...
{
    struct sway_view *view = wl_container_of(listener, view, foreign_destroy);
...
void
view_map(struct sway_view *view, struct wlr_surface *wlr_surface,
         bool fullscreen, struct wlr_output *fullscreen_output, bool decoration)
{
    ...
    if (node && node->type == N_CONTAINER) {
        if (container_is_floating(node->sway_container)) {
            // If we're about to launch the view into the floating container,
            // then launch it as a tiled view instead.
            if (ws) {
                target_sibling = seat_get_focus_inactive_tiling(seat, ws);
                if (target_sibling) {
                    struct sway_container *con = seat_get_focus_inactive_view(
                        seat, &target_sibling->node);
                    if (con) {
                        target_sibling = con;
                    }
                }
    ...
    if (view->impl->wants_floating && view->impl->wants_floating(view)) {
        ...
        view->container->pending.border_thickness =
...
void
view_unmap(struct sway_view *view)
{
...
void
view_update_size(struct sway_view *view)
{
    struct sway_container *con = view->container;
...
void
view_center_and_clip_surface(struct sway_view *view)
{
    struct sway_container *con = view->container;
    ...
    if (container_is_floating(con)) {
        ...
        int x = (int)fmax(
            0, (con->current.content_width - view->geometry.width) / 2);
        int y = (int)fmax(
            0, (con->current.content_height - view->geometry.height) / 2);
    ...
    if (!wl_list_empty(&con->view->content_tree->children)) {
        ...
        wlr_scene_subsurface_tree_set_clip(&con->view->content_tree->node,
                                           &clip);
    }
}
...
view_from_wlr_surface(struct wlr_surface *wlr_surface)
{
    ...
    sway_log(SWAY_DEBUG, "Surface of unknown type (role %s): %p", role,
             wlr_surface);
    return NULL;
}

void
view_update_app_id(struct sway_view *view)
{
...
void
view_update_title(struct sway_view *view, bool force)
{
...
bool
view_is_visible(struct sway_view *view)
{
    ...
    if (!workspace &&
        view->container->pending.fullscreen_mode != FULLSCREEN_GLOBAL) {
    ...
    while (con) {
        ...
        if ((layout == L_TABBED || layout == L_STACKED) &&
            ...
            struct sway_node *parent = con->pending.parent
                                           ? &con->pending.parent->node
                                           : &con->pending.workspace->node;
...
void
view_set_urgent(struct sway_view *view, bool enable)
{
...
bool
view_is_urgent(struct sway_view *view)
{
    return view->urgent.tv_sec || view->urgent.tv_nsec;
}

void
view_remove_saved_buffer(struct sway_view *view)
{
...
static void
view_save_buffer_iterator(struct wlr_scene_buffer *buffer, int sx, int sy,
                          void *data)
{
    ...
    if (!sbuf) {
        sway_log(SWAY_ERROR,
                 "Could not allocate a scene buffer when saving a surface");
        return;
    }
...
void
view_save_buffer(struct sway_view *view)
{
    ...
    if (!view->saved_surface_tree) {
        sway_log(SWAY_ERROR,
                 "Could not allocate a scene tree node when saving a surface");
        return;
    }
...
bool
view_is_transient_for(struct sway_view *child, struct sway_view *ancestor)
{
...
bool
view_can_tear(struct sway_view *view)
{
...
static void
send_frame_done_iterator(struct wlr_scene_buffer *scene_buffer, int x, int y,
                         void *data)
{
