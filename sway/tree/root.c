...
struct sway_root *
root_create(struct wl_display *wl_display)
{
    ...
    root->layers.fullscreen_global =
        alloc_scene_tree(root->layer_tree, &failed);
#if WLR_HAS_XWAYLAND
...
void
root_destroy(struct sway_root *root)
{
...
static void
set_container_transform(struct sway_workspace *ws, struct sway_container *con)
{
...
void
root_scratchpad_add_container(struct sway_container *con,
...
void
root_scratchpad_remove_container(struct sway_container *con)
{
...
void
root_scratchpad_show(struct sway_container *con)
{
    struct sway_seat *seat = input_manager_current_seat();
...
static void
disable_fullscreen(struct sway_container *con, void *data)
{
    if (con->pending.fullscreen_mode != FULLSCREEN_NONE) {
        container_fullscreen_disable(con);
    }
...
void
root_scratchpad_hide(struct sway_container *con)
{
    struct sway_seat *seat = input_manager_current_seat();
...
void
root_for_each_workspace(void (*f)(struct sway_workspace *ws, void *data),
                        void *data)
{
...
void
root_for_each_container(void (*f)(struct sway_container *con, void *data),
                        void *data)
{
    for (int i = 0; i < root->outputs->length; ++i) {
        struct sway_output *output = root->outputs->items[i];
...
struct sway_output *
root_find_output(bool (*test)(struct sway_output *output, void *data),
                 void *data)
{
    for (int i = 0; i < root->outputs->length; ++i) {
        struct sway_output *output = root->outputs->items[i];
...
void
root_get_box(struct sway_root *root, struct wlr_box *box)
{
    box->x = root->x;
    box->y = root->y;
