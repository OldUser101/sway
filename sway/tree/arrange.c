...
static void
apply_horiz_layout(list_t *children, struct wlr_box *parent)
{
    if (!children->length) {
        return;
    }
        ...
        if (child->width_fraction <= 0) {
            if (current_width_fraction <= 0) {
                ...
                child->width_fraction =
                    current_width_fraction / (children->length - new_children);
            } else {
    ...
    double total_gap =
        fmin(inner_gap * (children->length - 1),
...
static void
apply_vert_layout(list_t *children, struct wlr_box *parent)
{
    if (!children->length) {
        return;
    }
    ...
    for (int i = 0; i < children->length; ++i) {
        ...
        if (i == children->length - 1) {
            child->pending.height =
                parent->y + parent->height - child->pending.y;
        }
...
static void
apply_tabbed_layout(list_t *children, struct wlr_box *parent)
{
    if (!children->length) {
        return;
    }
...
static void
apply_stacked_layout(list_t *children, struct wlr_box *parent)
{
    if (!children->length) {
        return;
    }
    for (int i = 0; i < children->length; ++i) {
        struct sway_container *child = children->items[i];
        int parent_offset =
            child->view ? 0 : container_titlebar_height() * children->length;
        child->pending.x = parent->x;
        child->pending.y = parent->y + parent_offset;
        child->pending.width = parent->width;
        child->pending.height = parent->height - parent_offset;
...
static void
arrange_floating(list_t *floating)
{
...
static void
arrange_children(list_t *children, enum sway_container_layout layout,
...
void
arrange_container(struct sway_container *container)
{
    if (config->reloading) {
        return;
    }
    ...
    arrange_children(container->pending.children, container->pending.layout,
                     &box);
    node_set_dirty(&container->node);
}

void
arrange_workspace(struct sway_workspace *workspace)
{
    if (config->reloading) {
        return;
    }
    ...
    sway_log(SWAY_DEBUG, "Usable area for ws: %dx%d@%d,%d", area->width,
    ...
    sway_log(SWAY_DEBUG, "Arranging workspace '%u' at %f, %f", workspace->num,
...
void
arrange_output(struct sway_output *output)
{
    if (config->reloading) {
        return;
    }
...
void
arrange_node(struct sway_node *node)
{
    switch (node->type) {
        case N_ROOT:
            arrange_root();
