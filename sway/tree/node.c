...
void
node_init(struct sway_node *node, enum sway_node_type type, void *thing)
{
...
const char *
node_type_to_str(enum sway_node_type type)
{
    switch (type) {
        case N_ROOT:
            return "root";
...
void
node_set_dirty(struct sway_node *node)
{
    if (node->dirty) {
        return;
    }
...
bool
node_is_view(struct sway_node *node)
{
    return node->type == N_CONTAINER && node->sway_container->view;
}

char *
node_get_name(struct sway_node *node)
{
    switch (node->type) {
        case N_ROOT:
            return strdup("root");
        case N_OUTPUT:
            return strdup(node->sway_output->wlr_output->name);
        case N_WORKSPACE:
            return format_str("workspace %d", node->sway_workspace->num);
        case N_CONTAINER:
            return strdup(node->sway_container->title);
    }
    return NULL;
}

void
node_get_box(struct sway_node *node, struct wlr_box *box)
{
    switch (node->type) {
        case N_ROOT:
...
struct sway_output *
node_get_output(struct sway_node *node)
{
    switch (node->type) {
        case N_CONTAINER: {
...
node_get_layout(struct sway_node *node)
{
    switch (node->type) {
        case N_CONTAINER:
...
struct sway_node *
node_get_parent(struct sway_node *node)
{
    switch (node->type) {
        case N_CONTAINER: {
...
list_t *
node_get_children(struct sway_node *node)
{
    switch (node->type) {
        case N_CONTAINER:
...
bool
node_has_ancestor(struct sway_node *node, struct sway_node *ancestor)
{
    ...
    while (parent) {
        ...
        if (ancestor->type == N_ROOT && parent->type == N_CONTAINER &&
            parent->sway_container->pending.fullscreen_mode ==
                FULLSCREEN_GLOBAL) {
            return true;
        }
...
void
scene_node_disown_children(struct wlr_scene_tree *tree)
{
    // this function can be called as part of destruction code that will be
    // invoked upon an allocation failure. Let's not crash on NULL due to an
    // allocation error.
    if (!tree) {
        return;
    }
...
alloc_scene_tree(struct wlr_scene_tree *parent, bool *failed)
{
    // fallthrough
    if (*failed) {
        return NULL;
