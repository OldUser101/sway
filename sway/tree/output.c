...
static void
restore_workspaces(struct sway_output *output)
{
    ...
    for (int i = 0; i < root->outputs->length; i++) {
        ...
        if (other->workspaces->length == 0) {
            int next = workspace_next_num(other->wlr_output->name);
...
static void
destroy_scene_layers(struct sway_output *output)
{
...
void
output_enable(struct sway_output *output)
{
    ...
    if (!output->workspaces->length) {
        // Create workspace
        int ws_num = workspace_next_num(wlr_output->name);
        sway_log(SWAY_DEBUG, "Creating default workspace %u", ws_num);
        ws = workspace_create(output, ws_num);
...
static void
evacuate_sticky(struct sway_workspace *old_ws, struct sway_output *new_output)
{
...
static void
output_evacuate(struct sway_output *output)
{
    if (!output->workspaces->length) {
        return;
    }
...
void
output_destroy(struct sway_output *output)
{
    if (!sway_assert(
    ...
    if (!sway_assert(output->node.ntxnrefs == 0,
...
void
output_disable(struct sway_output *output)
{
...
void
output_begin_destroy(struct sway_output *output)
{
...
struct sway_output *
output_get_in_direction(struct sway_output *reference,
...
void
output_add_workspace(struct sway_output *output,
...
void
output_for_each_workspace(struct sway_output *output,
                          void (*f)(struct sway_workspace *ws, void *data),
                          void *data)
{
...
void
output_for_each_container(struct sway_output *output,
...
output_find_workspace(struct sway_output *output,
                      bool (*test)(struct sway_workspace *ws, void *data),
                      void *data)
{
    for (int i = 0; i < output->workspaces->length; ++i) {
        struct sway_workspace *workspace = output->workspaces->items[i];
...
output_find_container(struct sway_output *output,
...
void
output_sort_workspaces(struct sway_output *output)
{
    list_stable_sort(output->workspaces, sort_workspace_cmp_qsort);
}

void
output_get_box(struct sway_output *output, struct wlr_box *box)
{
    box->x = output->lx;
    box->y = output->ly;
...
static void
handle_destroy_non_desktop(struct wl_listener *listener, void *data)
{
    struct sway_output_non_desktop *output =
        wl_container_of(listener, output, destroy);

    sway_log(SWAY_DEBUG, "Destroying non-desktop output '%s'",
...
output_non_desktop_create(struct wlr_output *wlr_output)
{
    struct sway_output_non_desktop *output =
