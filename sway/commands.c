...
checkarg(int argc, const char *name, enum expected_args type, int val)
{
    ...
    return error_name ? cmd_results_new(CMD_INVALID,
                                        "Invalid %s command "
...
find_handler(const char *line, const struct cmd_handler *handlers,
             size_t handlers_size)
{
    ...
    return bsearch(&query, handlers, handlers_size / sizeof(struct cmd_handler),
                   sizeof(struct cmd_handler), handler_compare);
}

static const struct cmd_handler *
find_handler_ex(char *line, const struct cmd_handler *config_handlers,
                size_t config_handlers_size,
                const struct cmd_handler *command_handlers,
                size_t command_handlers_size,
...
find_core_handler(char *line)
{
    return find_handler_ex(line, config_handlers, sizeof(config_handlers),
                           command_handlers, sizeof(command_handlers), handlers,
                           sizeof(handlers));
}

static void
set_config_node(struct sway_node *node, bool node_overridden)
{
...
list_t *
execute_command(char *_exec, struct sway_seat *seat, struct sway_container *con)
{
    char *cmd;
    char matched_delim = ';';
...
config_command(char *exec, char **new_block)
{
    ...
    if (!handler || !handler->handle) {
        if (handler) {
            results = cmd_results_new(
                CMD_INVALID, "Command '%s' is shimmed, but unimplemented",
                argv[0]);
        } else {
...
struct cmd_results *
config_subcommand(char **argv, int argc, const struct cmd_handler *handlers,
                  size_t handlers_size)
{
    ...
    if (!handler) {
        return cmd_results_new(CMD_INVALID, "Unknown/invalid command '%s'",
                               argv[0]);
    }
    if (handler->handle) {
    ...
    for (int i = 0; i < res_list->length; ++i) {
        ...
        if (results->error) {
            ...
            json_object_object_add(root, "error",
                                   json_object_new_string(results->error));
        }
