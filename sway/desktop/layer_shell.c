...
toplevel_layer_surface_from_surface(struct wlr_surface *surface)
{
...
static void
arrange_surface(struct sway_output *output, const struct wlr_box *full_area,
                ...
                bool exclusive)
{
    ...
    wl_list_for_each(node, &tree->children, link)
    {
        struct sway_layer_surface *surface =
            scene_descriptor_try_get(node, SWAY_SCENE_DESC_LAYER_SHELL);
        ...
        if ((surface->scene->layer_surface->current.exclusive_zone > 0) !=
            exclusive) {
            continue;
        }
...
void
arrange_layers(struct sway_output *output)
{
    ...
    arrange_surface(output, &full_area, &usable_area,
                    output->layers.shell_overlay, true);
    arrange_surface(output, &full_area, &usable_area, output->layers.shell_top,
                    true);
    arrange_surface(output, &full_area, &usable_area,
                    output->layers.shell_bottom, true);
    arrange_surface(output, &full_area, &usable_area,
                    output->layers.shell_background, true);

    arrange_surface(output, &full_area, &usable_area,
                    output->layers.shell_overlay, false);
    arrange_surface(output, &full_area, &usable_area, output->layers.shell_top,
                    false);
    arrange_surface(output, &full_area, &usable_area,
                    output->layers.shell_bottom, false);
    arrange_surface(output, &full_area, &usable_area,
                    output->layers.shell_background, false);
...
sway_layer_get_scene(struct sway_output *output,
...
static struct sway_layer_surface *
sway_layer_surface_create(struct wlr_scene_layer_surface_v1 *scene)
{
    ...
    if (!scene_descriptor_assign(&popups->node, SWAY_SCENE_DESC_POPUP,
                                 &surface->desc)) {
...
static struct sway_layer_surface *
find_mapped_layer_by_client(struct wl_client *client,
                            struct sway_output *ignore_output)
{
...
handle_surface_commit(struct wl_listener *listener, void *data)
{
    ...
    if (layer_surface->initialized &&
        ...
        struct wlr_scene_tree *output_layer =
            sway_layer_get_scene(surface->output, layer_type);
...
static void
popup_unconstrain(struct sway_layer_popup *popup)
{
...
static struct sway_layer_popup *
...
handle_layer_shell_surface(struct wl_listener *listener, void *data)
{
    ...
    struct wlr_scene_tree *output_layer =
        sway_layer_get_scene(output, layer_type);
