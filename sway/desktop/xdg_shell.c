...
{
    struct sway_xdg_popup *popup = wl_container_of(listener, popup, new_popup);
...
static void
popup_unconstrain(struct sway_xdg_popup *popup)
{
...
static struct sway_xdg_popup *
popup_create(struct wlr_xdg_popup *wlr_popup, struct sway_view *view,
             struct wlr_scene_tree *parent)
{
...
static struct sway_xdg_shell_view *
xdg_shell_view_from_view(struct sway_view *view)
{
...
static void
get_constraints(struct sway_view *view, double *min_width, double *max_width,
...
static const char *
get_string_prop(struct sway_view *view, enum sway_view_prop prop)
{
...
static uint32_t
configure(struct sway_view *view, double lx, double ly, int width, int height)
{
...
static void
set_activated(struct sway_view *view, bool activated)
{
    if (xdg_shell_view_from_view(view) == NULL) {
        return;
    }
...
static void
set_tiled(struct sway_view *view, bool tiled)
{
    if (xdg_shell_view_from_view(view) == NULL) {
        return;
    }
    if (wl_resource_get_version(view->wlr_xdg_toplevel->resource) >=
        ...
        // The version is too low for the tiled state; configure as maximized
        // instead to stop the client from drawing decorations outside of the
        // toplevel geometry.
...
static void
set_fullscreen(struct sway_view *view, bool fullscreen)
{
    if (xdg_shell_view_from_view(view) == NULL) {
        return;
    }
...
static void
set_resizing(struct sway_view *view, bool resizing)
{
    if (xdg_shell_view_from_view(view) == NULL) {
        return;
    }
...
static bool
wants_floating(struct sway_view *view)
{
...
static bool
is_transient_for(struct sway_view *child, struct sway_view *ancestor)
{
...
static void
close_popups(struct sway_view *view)
{
    struct wlr_xdg_popup *popup, *tmp;
    wl_list_for_each_safe(popup, tmp, &view->wlr_xdg_toplevel->base->popups,
                          link)
    {
...
handle_commit(struct wl_listener *listener, void *data)
{
    ...
    if (xdg_surface->initial_commit) {
        ...
        wlr_xdg_toplevel_set_wm_capabilities(
            view->wlr_xdg_toplevel, XDG_TOPLEVEL_WM_CAPABILITIES_FULLSCREEN);
    ...
    if (new_size) {
        ...
        if (container_is_floating(view->container)) {
            ...
            // Only set the toplevel size the current container actually has a
            // size.
            if (view->container->current.width) {
                wlr_xdg_toplevel_set_size(view->wlr_xdg_toplevel,
                                          view->geometry.width,
    ...
    if (view->container->node.instruction) {
        bool successful = transaction_notify_view_ready_by_serial(
...
handle_map(struct wl_listener *listener, void *data)
{
    ...
    view_map(view, toplevel->base->surface, toplevel->requested.fullscreen,
...
void
handle_xdg_shell_toplevel(struct wl_listener *listener, void *data)
{
    ...
    wlr_scene_xdg_surface_create(xdg_shell_view->view.content_tree,
                                 xdg_toplevel->base);
