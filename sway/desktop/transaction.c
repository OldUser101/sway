...
static struct sway_transaction *
...
static void
transaction_destroy(struct sway_transaction *transaction)
{
    // Free instructions
...
static void
copy_output_state(struct sway_output *output,
                  struct sway_transaction_instruction *instruction)
{
...
static void
copy_workspace_state(struct sway_workspace *ws,
                     struct sway_transaction_instruction *instruction)
{
...
static void
copy_container_state(struct sway_container *container,
                     struct sway_transaction_instruction *instruction)
{
...
static void
transaction_add_node(struct sway_transaction *transaction,
...
static void
apply_output_state(struct sway_output *output, struct sway_output_state *state)
{
...
static void
apply_workspace_state(struct sway_workspace *ws,
...
static void
apply_container_state(struct sway_container *container,
...
static void
arrange_title_bar(struct sway_container *con, int x, int y, int width,
                  int height)
{
...
static void
disable_container(struct sway_container *con)
{
    if (con->view) {
        ...
        for (int i = 0; i < con->current.children->length; i++) {
            ...
            wlr_scene_node_reparent(&child->scene_tree->node,
                                    con->content_tree);
...
static void arrange_container(struct sway_container *con, int width, int height,
                              bool title_bar, int gaps);

static void
arrange_children(enum sway_container_layout layout, list_t *children,
                 ...
                 int width, int height, int gaps)
{
    ...
    if (layout == L_TABBED) {
        struct sway_container *first =
            children->length == 1
                ? ((struct sway_container *)children->items[0])
                : NULL;
        ...
        for (int i = 0; i < children->length; i++) {
            ...
            arrange_title_bar(child, 0, y - title_height, width,
                              title_bar_height);
            wlr_scene_node_set_enabled(&child->border.tree->node, activated);
            wlr_scene_node_set_enabled(&child->scene_tree->node, true);
            wlr_scene_node_set_position(&child->scene_tree->node, 0,
...
static void
arrange_container(struct sway_container *con, int width, int height,
                  bool title_bar, int gaps)
{
    ...
    if (con->view) {
        ...
        if (con->current.border == B_NORMAL) {
            ...
            container_update(con);
            border_top =
                title_bar && con->current.border_top ? border_width : 0;
        ...
        wlr_scene_rect_set_size(con->border.left, border_left,
                                vert_border_height);
        wlr_scene_rect_set_size(con->border.right, border_right,
                                vert_border_height);
...
static int
container_get_gaps(struct sway_container *con)
{
...
static void
arrange_fullscreen(struct wlr_scene_tree *tree, struct sway_container *fs,
                   struct sway_workspace *ws, int width, int height)
{
...
static void
arrange_workspace_floating(struct sway_workspace *ws)
{
    for (int i = 0; i < ws->current.floating->length; i++) {
        ...
        if (root->fullscreen_global) {
            ...
            for (int i = 0; i < root->outputs->length; i++) {
                ...
                struct sway_workspace *active =
                    output->current.active_workspace;
        ...
        arrange_container(floater, floater->current.width,
                          floater->current.height, true, ws->gaps_inner);
    }
}

static void
arrange_workspace_tiling(struct sway_workspace *ws, int width, int height)
{
...
static void
disable_workspace(struct sway_workspace *ws)
{
...
static void
arrange_output(struct sway_output *output, int width, int height)
{
    for (int i = 0; i < output->current.workspaces->length; i++) {
        ...
        wlr_scene_node_reparent(&child->layers.tiling->node,
        ...
        if (activated) {
            ...
            if (fs) {
                ...
                wlr_scene_rect_set_size(output->fullscreen_background, width,
                                        height);

                arrange_workspace_floating(child);
                arrange_fullscreen(child->layers.fullscreen, fs, child, width,
                                   height);
            } else {
                ...
                arrange_workspace_tiling(
                    child, area->width - gaps->left - gaps->right,
...
void
arrange_popups(struct wlr_scene_tree *popups)
{
...
static void
arrange_root(struct sway_root *root)
{
    ...
    for (int i = 0; i < root->scratchpad->length; i++) {
        ...
        // children so that disabling the container will disable all
        // descendants.
        if (!con->view)
            for (int ii = 0; ii < con->current.children->length; ii++) {
    ...
    if (fs) {
        for (int i = 0; i < root->outputs->length; i++) {
            ...
            for (int j = 0; j < output->current.workspaces->length; j++) {
                struct sway_workspace *workspace =
                    output->current.workspaces->items[j];
        ...
        for (int i = 0; i < root->outputs->length; i++) {
            ...
            wlr_scene_node_reparent(&output->layers.shell_background->node,
                                    root->layers.shell_background);
            wlr_scene_node_reparent(&output->layers.shell_bottom->node,
                                    root->layers.shell_bottom);
            wlr_scene_node_reparent(&output->layers.tiling->node,
                                    root->layers.tiling);
            wlr_scene_node_reparent(&output->layers.shell_top->node,
                                    root->layers.shell_top);
            wlr_scene_node_reparent(&output->layers.shell_overlay->node,
                                    root->layers.shell_overlay);
            wlr_scene_node_reparent(&output->layers.fullscreen->node,
                                    root->layers.fullscreen);
            wlr_scene_node_reparent(&output->layers.session_lock->node,
                                    root->layers.session_lock);

            wlr_scene_node_set_position(&output->layers.shell_background->node,
                                        output->lx, output->ly);
            wlr_scene_node_set_position(&output->layers.shell_bottom->node,
                                        output->lx, output->ly);
            wlr_scene_node_set_position(&output->layers.tiling->node,
                                        output->lx, output->ly);
            wlr_scene_node_set_position(&output->layers.fullscreen->node,
                                        output->lx, output->ly);
            wlr_scene_node_set_position(&output->layers.shell_top->node,
                                        output->lx, output->ly);
            wlr_scene_node_set_position(&output->layers.shell_overlay->node,
                                        output->lx, output->ly);
            wlr_scene_node_set_position(&output->layers.session_lock->node,
                                        output->lx, output->ly);
 ...
 */
static void
transaction_apply(struct sway_transaction *transaction)
{
    ...
    for (int i = 0; i < transaction->instructions->length; ++i) {
        ...
        switch (node->type) {
            case N_ROOT:
                break;
            case N_OUTPUT:
                apply_output_state(node->sway_output,
...
handle_timeout(void *data)
{
    ...
    sway_log(SWAY_DEBUG, "Transaction %p timed out (%zi waiting)", transaction,
...
static bool
should_configure(struct sway_node *node,
                 struct sway_transaction_instruction *instruction)
{
...
static void
transaction_commit(struct sway_transaction *transaction)
{
    ...
    for (int i = 0; i < transaction->instructions->length; ++i) {
        ...
        if (should_configure(node, instruction)) {
            instruction->serial =
                view_configure(node->sway_container->view,
...
static void
set_instruction_ready(struct sway_transaction_instruction *instruction)
{
...
bool
transaction_notify_view_ready_by_serial(struct sway_view *view, uint32_t serial)
{
    struct sway_transaction_instruction *instruction =
...
bool
transaction_notify_view_ready_by_geometry(struct sway_view *view, double x,
