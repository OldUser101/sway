...
void
launcher_ctx_consume(struct launcher_ctx *ctx)
{
...
void
launcher_ctx_destroy(struct launcher_ctx *ctx)
{
    if (ctx == NULL) {
        return;
    }
...
struct launcher_ctx *
launcher_ctx_find_pid(pid_t pid)
{
    ...
    do {
        ...
        wl_list_for_each(_ctx, &server.pending_launcher_ctxs, link)
        {
            if (pid == _ctx->pid) {
                ctx = _ctx;
                char *name = node_get_name(ctx->node);
                sway_log(SWAY_DEBUG, "found %s match for pid %d: %s",
                         node_type_to_str(ctx->node->type), pid, name);
                free(name);
                break;
            }
        }
...
launcher_ctx_get_workspace(struct launcher_ctx *ctx)
{
    ...
    switch (ctx->node->type) {
        ...
        case N_OUTPUT:
            ...
            ws = workspace_by_number(ctx->fallback_num);
            if (!ws) {
                sway_log(
                    SWAY_DEBUG,
                    "Creating workspace %u for pid %d because it disappeared",
                    ctx->fallback_num, ctx->pid);
                if (!output->enabled) {
                    sway_log(
                        SWAY_DEBUG,
                ...
                ws = workspace_create(output, ctx->fallback_num);
            }
            break;
        case N_ROOT:
            ws = workspace_create(NULL, ctx->fallback_num);
            break;
    }
...
static void
ctx_handle_node_destroy(struct wl_listener *listener, void *data)
{
    ...
    switch (ctx->node->type) {
        ...
        case N_WORKSPACE:;
            ...
            // We want to save this ws num to recreate later, hopefully on the
            // same output
            ctx->fallback_num = ws->num;
            if (!ws->output || ws->output->node.destroying) {
                // If the output is being destroyed it would be pointless to
                // track If the output is being disabled, we'll find out if it's
                // still disabled when we try to match it.
                ctx->node = &root->node;
                break;
            }
...
static void
token_handle_destroy(struct wl_listener *listener, void *data)
{
...
static void
launch_ctx_handle_seat_destroy(struct wl_listener *listener, void *data)
{
...
struct launcher_ctx *
launcher_ctx_create_internal(void)
{
...
const char *
launcher_ctx_get_token_name(struct launcher_ctx *ctx)
{
