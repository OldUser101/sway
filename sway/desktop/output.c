...
bool
output_match_name_or_id(struct sway_output *output, const char *name_or_id)
{
...
output_get_active_workspace(struct sway_output *output)
{
...
static void
handle_buffer_timer_destroy(struct wl_listener *listener, void *data)
{
...
buffer_timer_get_or_create(struct wlr_scene_surface *scene_surface)
{
    ...
    timer->frame_done_timer = wl_event_loop_add_timer(
        server.wl_event_loop, handle_buffer_timer, scene_surface);
...
static void
send_frame_done_iterator(struct wlr_scene_buffer *buffer, int x, int y,
                         void *user_data)
{
...
get_scale_filter(struct sway_output *output, struct wlr_scene_buffer *buffer)
{
...
void
output_configure_scene(struct sway_output *output, struct wlr_scene_node *node,
                       float opacity)
{
    if (!node->enabled) {
        return;
    ...
    if (node->type == WLR_SCENE_NODE_BUFFER) {
        ...
        // hack: don't call the scene setter because that will damage all
        // outputs We don't want to damage outputs that aren't our current
        // output that we're configuring
        if (output) {
...
static bool
output_can_tear(struct sway_output *output)
{
...
output_repaint_timer_handler(void *data)
{
    ...
    if (!wlr_output_commit_state(output->wlr_output, &pending)) {
        sway_log(SWAY_ERROR, "Page-flip failed on output %s",
                 output->wlr_output->name);
    }
    wlr_output_state_finish(&pending);
    return 0;
...
void
update_output_manager_config(struct sway_server *server)
{
...
static int
timer_modeset_handle(void *data)
{
...
void
request_modeset(void)
{
    if (server.delayed_modeset == NULL) {
        server.delayed_modeset = wl_event_loop_add_timer(
            server.wl_event_loop, timer_modeset_handle, &server);
        wl_event_source_timer_update(server.delayed_modeset, 10);
    }
}

bool
modeset_is_pending(void)
{
...
static void
begin_destroy(struct sway_output *output)
{
...
static void
handle_layout_destroy(struct wl_listener *listener, void *data)
{
    struct sway_output *output =
        wl_container_of(listener, output, layout_destroy);
    begin_destroy(output);
}
...
handle_new_output(struct wl_listener *listener, void *data)
{
    ...
    sway_log(SWAY_DEBUG, "New output %p: %s (non-desktop: %d)", wlr_output,
    ...
    wl_signal_add(&root->output_layout->events.destroy,
...
output_config_for_config_head(
    struct wlr_output_configuration_head_v1 *config_head)
{
    struct output_config *oc =
        new_output_config(config_head->state.output->name);
    if (!oc) {
        return NULL;
    }
...
static void
output_manager_apply(struct sway_server *server,
                     struct wlr_output_configuration_v1 *cfg, bool test_only)
{
    bool ok = false;
    size_t configs_len =
        config->output_configs->length + wl_list_length(&cfg->heads);
