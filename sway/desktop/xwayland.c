...
static struct sway_xwayland_view *
xwayland_view_from_view(struct sway_view *view)
{
...
static uint32_t
get_int_prop(struct sway_view *view, enum sway_view_prop prop)
{
    if (xwayland_view_from_view(view) == NULL) {
        return 0;
    }
...
handle_commit(struct wl_listener *listener, void *data)
{
    ...
    if (view->container->node.instruction) {
        bool successful = transaction_notify_view_ready_by_geometry(
...
static void
handle_surface_tree_destroy(struct wl_listener *listener, void *data)
{
    struct sway_xwayland_view *xwayland_view =
...
handle_map(struct wl_listener *listener, void *data)
{
    ...
    if (xwayland_view->surface_tree) {
        xwayland_view->surface_tree_destroy.notify =
...
handle_set_startup_id(struct wl_listener *listener, void *data)
{
    ...
    struct wlr_xdg_activation_token_v1 *token =
        wlr_xdg_activation_v1_find_token(server.xdg_activation_v1,
                                         xsurface->startup_id);
    if (token == NULL) {
