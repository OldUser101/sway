...
static void
destroy_inhibitor(struct sway_idle_inhibitor_v1 *inhibitor)
{
...
void
sway_idle_inhibit_v1_user_inhibitor_register(struct sway_view *view,
...
sway_idle_inhibit_v1_user_inhibitor_for_view(struct sway_view *view)
{
    struct sway_idle_inhibit_manager_v1 *manager =
        &server.idle_inhibit_manager_v1;
    struct sway_idle_inhibitor_v1 *inhibitor;
...
struct sway_idle_inhibitor_v1 *
sway_idle_inhibit_v1_application_inhibitor_for_view(struct sway_view *view)
{
    struct sway_idle_inhibit_manager_v1 *manager =
        &server.idle_inhibit_manager_v1;
    struct sway_idle_inhibitor_v1 *inhibitor;
...
void
sway_idle_inhibit_v1_user_inhibitor_destroy(
    struct sway_idle_inhibitor_v1 *inhibitor)
{
    if (!inhibitor) {
        return;
...
bool
sway_idle_inhibit_v1_is_active(struct sway_idle_inhibitor_v1 *inhibitor)
{
        ...
        case INHIBIT_IDLE_APPLICATION:;
            ...
            if (layer_surface) {
                ...
                return layer_surface->output &&
                       layer_surface->output->enabled && wlr_surface->mapped;
            }

            // If there is no view associated with the inhibitor, assume
            // invisible
...
void
sway_idle_inhibit_v1_check_active(void)
{
    struct sway_idle_inhibit_manager_v1 *manager =
        &server.idle_inhibit_manager_v1;
    struct sway_idle_inhibitor_v1 *inhibitor;
...
bool
sway_idle_inhibit_manager_v1_init(void)
{
    struct sway_idle_inhibit_manager_v1 *manager =
        &server.idle_inhibit_manager_v1;
