...
static void
handle_swaybar_client_destroy(struct wl_listener *listener, void *data)
{
...
static void
invoke_swaybar(struct bar_config *bar)
{
    int sockets[2];
    ...
    if (pid < 0) {
        ...
        // run custom swaybar
        char *const cmd[] = {bar->swaybar_command ? bar->swaybar_command
                                                  : "swaybar",
                             "-b", bar->id, NULL};
        execvp(cmd[0], cmd);
        _exit(EXIT_FAILURE);
...
void
load_swaybar(struct bar_config *bar)
{
    if (bar->client != NULL) {
        wl_client_destroy(bar->client);
    }
