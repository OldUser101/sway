...
void
output_get_identifier(char *identifier, size_t len, struct sway_output *output)
{
...
const char *
sway_output_scale_filter_to_string(enum scale_filter_mode scale_filter)
{
    switch (scale_filter) {
...
static void
supersede_output_config(struct output_config *dst, struct output_config *src)
{
    if (src->enabled != -1) {
        dst->enabled = -1;
    }
...
store_output_config(struct output_config *oc)
{
    ...
    struct sway_output *output =
        wildcard ? NULL : all_output_by_name_or_id(oc->name);
    ...
    sway_log(SWAY_DEBUG,
             ...
             "position %d,%d scale %f subpixel %s transform %d) (bg %s %s) "
             "(power %d) "
             "(max render time: %d) (allow tearing: %d)",
             oc->name, oc->enabled, oc->width, oc->height, oc->refresh_rate,
             oc->x, oc->y, oc->scale,
             sway_wl_output_subpixel_to_string(oc->subpixel), oc->transform,
             oc->background, oc->background_option, oc->power,
...
static void
set_mode(struct wlr_output *output, struct wlr_output_state *pending, int width,
...
static void
set_modeline(struct wlr_output *output, struct wlr_output_state *pending,
...
{
    if (render_format == DRM_FORMAT_XRGB2101010 ||
        render_format == DRM_FORMAT_XBGR2101010) {
        return RENDER_BIT_DEPTH_10;
    } else if (render_format == DRM_FORMAT_XRGB8888 ||
               render_format == DRM_FORMAT_ARGB8888) {
...
static bool
render_format_is_bgr(uint32_t fmt)
{
    return fmt == DRM_FORMAT_XBGR2101010 || fmt == DRM_FORMAT_XBGR8888;
}

static bool
output_config_is_disabling(struct output_config *oc)
{
    return oc && (!oc->enabled || oc->power == 0);
}

static void
queue_output_config(struct output_config *oc, struct sway_output *output,
                    struct wlr_output_state *pending)
{
    ...
    if (wlr_output->adaptive_sync_supported) {
        if (oc && oc->adaptive_sync != -1) {
            wlr_output_state_set_adaptive_sync_enabled(pending,
                                                       oc->adaptive_sync == 1);
        } else {
    ...
    if (oc && oc->render_bit_depth != RENDER_BIT_DEPTH_DEFAULT) {
        if (oc->render_bit_depth == RENDER_BIT_DEPTH_10 &&
            bit_depth_from_format(output->wlr_output->render_format) ==
                oc->render_bit_depth) {
            // 10-bit was set successfully before, try to save some tests by
            // reusing the format
...
static bool
finalize_output_config(struct output_config *oc, struct sway_output *output)
{
    if (output == root->fallback_output) {
        return false;
    }
    ...
    enum scale_filter_mode scale_filter_new =
        oc ? oc->scale_filter : SCALE_FILTER_DEFAULT;
    ...
    if (oc && oc->x != INT_MAX && oc->y != INT_MAX) {
        sway_log(SWAY_DEBUG, "Set %s position to %d, %d", oc->name, oc->x,
                 oc->y);
...
static void
output_update_position(struct sway_output *output)
{
    struct wlr_box output_box;
    wlr_output_layout_get_box(root->output_layout, output->wlr_output,
                              &output_box);
...
find_output_config_from_list(struct output_config **configs, size_t configs_len,
...
struct output_config *
find_output_config(struct sway_output *sway_output)
{
...
static bool
config_has_manual_mode(struct output_config *oc)
{
    if (!oc) {
        return false;
    }
...
static void
dump_output_state(struct wlr_output *wlr_output, struct wlr_output_state *state)
{
    sway_log(SWAY_DEBUG, "Output state for %s", wlr_output->name);
    if (state->committed & WLR_OUTPUT_STATE_ENABLED) {
        sway_log(SWAY_DEBUG, "    enabled:       %s",
...
static void
reset_output_state(struct wlr_output_state *state)
{
...
static void
clear_later_output_states(struct wlr_backend_output_state *states,
...
static bool
search_finish(struct search_context *ctx, size_t output_idx)
{
    ...
    return wlr_output_swapchain_manager_prepare(ctx->swapchain_mgr, ctx->states,
                                                ctx->configs_len) &&
           search_valid_config(ctx, output_idx + 1);
}

static bool
search_adaptive_sync(struct search_context *ctx, size_t output_idx)
{
...
static bool
search_mode(struct search_context *ctx, size_t output_idx)
{
    struct matched_output_config *cfg = &ctx->configs[output_idx];
    struct wlr_backend_output_state *backend_state = &ctx->states[output_idx];
    struct wlr_output_state *state = &backend_state->base;
...
static bool
search_render_format(struct search_context *ctx, size_t output_idx)
{
    struct matched_output_config *cfg = &ctx->configs[output_idx];
    struct wlr_backend_output_state *backend_state = &ctx->states[output_idx];
    struct wlr_output_state *state = &backend_state->base;
    ...
    uint32_t fmts[] = {
        DRM_FORMAT_XRGB2101010, DRM_FORMAT_XBGR2101010, DRM_FORMAT_XRGB8888,
        DRM_FORMAT_ARGB8888,    DRM_FORMAT_RGB565,      DRM_FORMAT_INVALID,
    };
    ...
    if (cfg->config &&
        cfg->config->render_bit_depth != RENDER_BIT_DEPTH_DEFAULT) {
...
static bool
search_valid_config(struct search_context *ctx, size_t output_idx)
{
...
static int
compare_matched_output_config_priority(const void *a, const void *b)
{
...
static void
sort_output_configs_by_priority(struct matched_output_config *configs,
                                size_t configs_len)
{
...
static bool
apply_resolved_output_configs(struct matched_output_config *configs,
                              ...
                              bool degrade_to_off)
{
    ...
    bool ok = wlr_output_swapchain_manager_prepare(&swapchain_mgr, states,
                                                   configs_len);
    if (!ok) {
        sway_log(SWAY_ERROR, "Requested backend configuration failed, "
                             "searching for valid fallbacks");
...
bool
apply_output_configs(struct output_config **ocs, size_t ocs_len, bool test_only,
                     bool degrade_to_off)
{
    ...
    wl_list_for_each(sway_output, &root->all_outputs, link)
    {
        ...
        config->config =
            find_output_config_from_list(ocs, ocs_len, sway_output);
    }

    sort_output_configs_by_priority(configs, configs_len);
    bool ok = apply_resolved_output_configs(configs, configs_len, test_only,
                                            degrade_to_off);
    for (size_t idx = 0; idx < configs_len; idx++) {
        struct matched_output_config *cfg = &configs[idx];
...
void
apply_stored_output_configs(void)
{
...
static void
handle_swaybg_client_destroy(struct wl_listener *listener, void *data)
{
...
_spawn_swaybg(char **command)
{
    ...
    if (!sway_set_cloexec(sockets[0], true) ||
        !sway_set_cloexec(sockets[1], true)) {
        return false;
    }
