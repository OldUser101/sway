...
static int
get_text_width(struct sway_text_node *props)
{
...
render_backing_buffer(struct text_buffer *buffer)
{
    ...
    cairo_set_source_rgba(cairo, background[0], background[1], background[2],
                          background[3]);
    ...
    cairo_move_to(cairo, 0,
                  (config->font_baseline - buffer->props.baseline) * scale);

    render_text(cairo, config->font_description, scale,
                buffer->props.pango_markup, "%s", buffer->text);
...
text_calc_size(struct text_buffer *buffer)
{
    ...
    wlr_scene_buffer_set_dest_size(buffer->buffer_node, get_text_width(props),
                                   props->height);
}

struct sway_text_node *
sway_text_node_create(struct wlr_scene_tree *parent, char *text, float color[4],
                      bool pango_markup)
{
...
void
sway_text_node_set_color(struct sway_text_node *node, float color[4])
{
...
void
sway_text_node_set_text(struct sway_text_node *node, char *text)
{
    struct text_buffer *buffer = wl_container_of(node, buffer, props);
...
void
sway_text_node_set_max_width(struct sway_text_node *node, int max_width)
{
    struct text_buffer *buffer = wl_container_of(node, buffer, props);
...
void
sway_text_node_set_background(struct sway_text_node *node, float background[4])
{
    struct text_buffer *buffer = wl_container_of(node, buffer, props);
