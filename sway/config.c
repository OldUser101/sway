                                ...
                                uint32_t context_flags)
{
    struct xkb_context *context =
        xkb_context_new(context_flags | XKB_CONTEXT_NO_SECURE_GETENV);
...
static void
free_mode(struct sway_mode *mode)
{
    if (!mode) {
        return;
    }
...
void
free_config(struct sway_config *config)
{
    if (!config) {
        return;
    }
...
static void
destroy_removed_seats(struct sway_config *old_config,
                      struct sway_config *new_config)
{
    ...
    for (i = 0; i < old_config->seat_configs->length; i++) {
        ...
        if (new_config && list_seq_find(new_config->seat_configs, seat_name_cmp,
                                        seat_config->name) < 0) {
...
static void
config_defaults(struct sway_config *config)
{
    if (!(config->swaynag_command = strdup("swaynag")))
        goto cleanup;
    config->swaynag_config_errors = (struct swaynag_instance){0};
    config->swaynag_config_errors.args =
        "--type error "
    ...
    if (!(config->current_mode->switch_bindings = create_list()))
        goto cleanup;
    if (!(config->current_mode->gesture_bindings = create_list()))
        goto cleanup;
    ...
    if (!(config->swaybg_command = strdup("swaybg")))
        goto cleanup;
    ...
    color_to_rgba(config->border_colors.focused_inactive.background,
                  0x5F676AFF);
    ...
    color_to_rgba(config->border_colors.focused_inactive.child_border,
                  0x5F676AFF);
    ...
    config->keysym_translation_state =
        keysym_translation_state_create(rules, 0);
    if (config->keysym_translation_state == NULL) {
        config->keysym_translation_state = keysym_translation_state_create(
            rules, XKB_CONTEXT_NO_ENVIRONMENT_NAMES);
    }
    if (config->keysym_translation_state == NULL) {
        goto cleanup;
    }
...
static bool
load_config(const char *path, struct sway_config *config,
...
static bool
load_include_config(const char *path, struct sway_config *config,
                    struct swaynag_instance *swaynag)
{
    // save parent config
...
void
load_include_configs(const char *path, struct sway_config *config,
                     struct swaynag_instance *swaynag)
{
    ...
    while (config->cmd_queue->length) {
        ...
        for (int i = 0; i < res_list->length; ++i) {
            ...
            if (res->status != CMD_SUCCESS) {
                sway_log(SWAY_ERROR, "Error on line '%s': %s", line,
                         res->error);
            }
...
expand_line(const char *block, const char *line, bool add_brace)
{
    ...
    snprintf(expanded, size, "%s%s%s%s", block ? block : "", block ? " " : "",
             line, add_brace ? " {" : "");
    return expanded;
}

bool
read_config(FILE *file, struct sway_config *config,
            struct swaynag_instance *swaynag)
{
    ...
    if (config->current_config == NULL) {
        ...
        if (this_config == NULL) {
            sway_log(SWAY_ERROR,
                     "Unable to allocate buffer for config contents");
            return false;
        }
    }
    ...
    size_t read = 0;
    int nlines = 0;
    while ((nread = getline_with_cont(&line, &line_size, file, &nlines)) !=
           -1) {
        ...
        if (line[strlen(line) - 1] != '{' && line[strlen(line) - 1] != '}') {
            ...
            if (brace_detected > 0) {
                ...
                sway_log(SWAY_DEBUG, "Detected open brace on line %d",
                         line_number);
            }
        }
        ...
        switch (res->status) {
            case CMD_FAILURE:
            case CMD_INVALID:
                sway_log(SWAY_ERROR, "Error on line %i '%s': %s (%s)",
                         line_number, line, res->error,
            ...
            case CMD_BLOCK_END:
                if (!block) {
                    sway_log(SWAY_DEBUG, "Unmatched '}' on line %i",
                             line_number);
                    success = false;
                    break;
                }
...
void
config_add_swaynag_warning(char *fmt, ...)
{
    if (config->reading && !config->validating) {
        ...
        swaynag_log(config->swaynag_command, &config->swaynag_config_errors,
                    ...
                    config->current_config_line_number,
                    config->current_config_path, config->current_config_line,
                    str);
    ...
    while ((find = strchr(find, '$'))) {
        ...
        for (i = 0; i < config->symbols->length; ++i) {
            ...
            if (has_prefix(find, var->name)) {
                ...
                if (!newstr) {
                    sway_log(SWAY_ERROR, "Unable to allocate replacement "
...
workspace_output_cmp_workspace(const void *a, const void *b)
{
    ...
    return wsa->workspace == wsb->workspace;
}
...
config_update_font_height(void)
{
    ...
    get_text_metrics(config->font_description, &config->font_height,
...
translate_keysyms(struct input_config *input_config)
{
    ...
    if (config->keysym_translation_state == NULL) {
        sway_log(SWAY_ERROR,
                 "Failed to create keysym translation XKB state "
                 "for device '%s'",
