...
static void
focus_surface(struct sway_session_lock *lock, struct wlr_surface *focused)
{
...
static void
refocus_output(struct sway_session_lock_output *output)
{
...
{
    struct sway_session_lock_output *surf =
...
handle_surface_destroy(struct wl_listener *listener, void *data)
{
    ...
    sway_assert(
        output->surface,
        "Trying to destroy a surface that the lock doesn't think exists");
    output->surface = NULL;
...
static void
lock_output_reconfigure(struct sway_session_lock_output *output)
{
...
handle_new_surface(struct wl_listener *listener, void *data)
{
    ...
    sway_assert(!lock_output->surface,
                "Tried to reassign a surface to an existing output");
    ...
    wl_signal_add(&lock_surface->surface->events.map,
                  &lock_output->surface_map);

    lock_output_reconfigure(lock_output);
}

static void
sway_session_lock_output_destroy(struct sway_session_lock_output *output)
{
    if (output->surface) {
        refocus_output(output);
        wl_list_remove(&output->surface_destroy.link);
...
static void
lock_node_handle_destroy(struct wl_listener *listener, void *data)
{
    struct sway_session_lock_output *output =
...
session_lock_output_create(struct sway_session_lock *lock,
                           struct sway_output *output)
{
    struct sway_session_lock_output *lock_output =
    ...
    struct wlr_scene_tree *tree =
        wlr_scene_tree_create(output->layers.session_lock);
    if (!tree) {
        sway_log(SWAY_ERROR,
                 "failed to allocate a session lock output scene tree");
        free(lock_output);
        return NULL;
    }

    struct wlr_scene_rect *background =
        wlr_scene_rect_create(tree, 0, 0,
                              (float[4]){
...
static void
sway_session_lock_destroy(struct sway_session_lock *lock)
{
...
void
sway_session_lock_add_output(struct sway_session_lock *lock,
                             struct sway_output *output)
{
    struct sway_session_lock_output *lock_output =
...
bool
sway_session_lock_has_surface(struct sway_session_lock *lock,
