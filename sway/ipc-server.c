...
void
ipc_init(struct sway_server *server)
{
    ...
    if (getenv("SWAYSOCK") != NULL && access(getenv("SWAYSOCK"), F_OK) == -1) {
        strncpy(ipc_sockaddr->sun_path, getenv("SWAYSOCK"),
    ...
    ipc_event_source =
        wl_event_loop_add_fd(server->wl_event_loop, ipc_socket,
...
ipc_user_sockaddr(void)
{
    ...
    if (path_size <= snprintf(ipc_sockaddr->sun_path, path_size,
                              "%s/sway-ipc.%u.%i.sock", dir, getuid(),
                              getpid())) {
...
ipc_client_handle_readable(int client_fd, uint32_t mask, void *data)
{
    ...
    memcpy(&client->pending_type, buf + sizeof(ipc_magic) + sizeof(uint32_t),
           sizeof(uint32_t));
...
void
ipc_event_workspace(struct sway_workspace *old, struct sway_workspace *new,
                    const char *change)
{
...
void
ipc_event_window(struct sway_container *window, const char *change)
{
...
ipc_event_binding(struct sway_binding *binding)
{
    ...
    json_object_object_add(json_binding, "command",
    ...
    switch (binding->type) {
        case BINDING_KEYCODE:; // bindcode: populate input_codes
            ...
            for (int i = 0; i < binding->keys->length; ++i) {
                ...
                json_object_array_add(input_codes,
                                      json_object_new_int(keycode));
                if (i == 0) {
                    input_code = keycode;
                }
            }
    ...
    json_object_object_add(json_binding, "input_code",
                           json_object_new_int(input_code));
    ...
    json_object_object_add(json_binding, "input_type",
...
void
ipc_event_input(const char *change, struct sway_input_device *device)
{
...
ipc_event_output(void)
{
    ...
    json_object *json = json_object_new_object();
    json_object_object_add(json, "change",
                           json_object_new_string("unspecified"));
...
ipc_client_handle_writable(int client_fd, uint32_t mask, void *data)
{
    ...
    if (written == -1 && errno == EAGAIN) {
        ...
        sway_log_errno(SWAY_INFO,
                       "Unable to send data from queue to IPC client");
        ipc_client_disconnect(client);
        return 0;
    }
...
static void
ipc_get_workspaces_callback(struct sway_workspace *workspace, void *data)
{
...
static void
ipc_get_marks_callback(struct sway_container *con, void *data)
{
...
static void
ipc_get_dyn_ws_global_max_callback(struct sway_workspace *workspace, void *data)
{
    int *next = (int *)data;
    if (workspace->num > *next)
        *next = workspace->num;
}

/// get the maximum output workspace number
static void
ipc_get_dyn_ws_local_max_callback(struct sway_workspace *workspace, void *data)
{
    struct sway_seat *seat = input_manager_get_default_seat();
    struct sway_workspace *focused_ws = seat_get_focused_workspace(seat);
...
static void
ipc_get_dyn_ws_find_next_callback(struct sway_workspace *workspace, void *data)
{
    struct sway_seat *seat = input_manager_get_default_seat();
    struct sway_workspace *focused_ws = seat_get_focused_workspace(seat);
    ...
    if (workspace->num <= next[0])
        return;

    if (workspace->num - next[0] < next[1] - next[0])
        next[1] = workspace->num;
}

/// find the previous workspace number in series
static void
ipc_get_dyn_ws_find_prev_callback(struct sway_workspace *workspace, void *data)
{
    struct sway_seat *seat = input_manager_get_default_seat();
    struct sway_workspace *focused_ws = seat_get_focused_workspace(seat);
    ...
    if (next[0] - workspace->num < next[0] - next[1])
        next[1] = workspace->num;
}

void
ipc_client_handle_command(struct ipc_client *client, uint32_t payload_length,
                          enum ipc_command_type payload_type)
{
    if (!sway_assert(client != NULL, "client != NULL")) {
        return;
    ...
    if (payload_length > 0) {
        ...
        if (received == -1) {
            sway_log_errno(SWAY_INFO,
                           "Unable to receive payload from IPC client");
            ipc_client_disconnect(client);
            free(buf);
            return;
        }
    ...
    switch (payload_type) {
        case IPC_COMMAND: {
            ...
            if (modeset_is_pending()) {
                // IPC expects commands to have taken immediate effect, so we
                // need to force a modeset after output commands. We do a single
                // modeset here to avoid modesetting for every output command in
                // sequence.
                force_modeset();
            }
        ...
        case IPC_GET_DYNAMIC_WORKSPACES: {
            ...
            if (next[1] == INT_MAX) {
                int global_max = 0;
                root_for_each_workspace(ipc_get_dyn_ws_global_max_callback,
                                        &global_max);
                next[1] = global_max + 1;
            }
            ...
            if (prev[1] == 0) {
                int local_max = 0;
                root_for_each_workspace(ipc_get_dyn_ws_local_max_callback,
                                        &local_max);
                prev[1] = local_max;
            }

            json_object *dyn_ws = json_object_new_object();
            json_object_object_add(dyn_ws, "current",
                                   json_object_new_int(current));
            json_object_object_add(dyn_ws, "next",
                                   json_object_new_int(next[1]));
            json_object_object_add(dyn_ws, "prev",
                                   json_object_new_int(prev[1]));
        ...
        case IPC_GET_OUTPUTS: {
            ...
            for (int i = 0; i < root->outputs->length; ++i) {
                ...
                const char *subpixel = sway_wl_output_subpixel_to_string(
            ...
            for (int i = 0; i < root->non_desktop_outputs->length; i++) {
                struct sway_output_non_desktop *non_desktop_output =
                    root->non_desktop_outputs->items[i];
                json_object_array_add(
                    outputs,
        ...
        case IPC_SUBSCRIBE: {
            ...
            if (request == NULL ||
                !json_object_is_type(request, json_type_array)) {
        ...
        case IPC_GET_BAR_CONFIG: {
            if (!buf[0]) {
                ...
                for (int i = 0; i < config->bars->length; ++i) {
                    ...
                    json_object_array_add(bars,
                                          json_object_new_string(bar->id));
                }
                ...
                if (!bar) {
                    const char *error = "{ \"success\": false, \"error\": \"No "
                                        "bar with that ID\" }";
        ...
        case IPC_GET_BINDING_MODES: {
            ...
            for (int i = 0; i < config->modes->length; i++) {
                ...
                json_object_array_add(modes,
                                      json_object_new_string(mode->name));
            }
        ...
        case IPC_SYNC: {
            // It was decided sway will not support this, just return
            // success:false
            const char msg[] = "{\"success\": false}";
            ipc_send_reply(client, payload_type, msg, strlen(msg));
            goto exit_cleanup;
...
bool
ipc_send_reply(struct ipc_client *client, enum ipc_command_type payload_type,
