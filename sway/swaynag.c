...
static void
handle_swaynag_client_destroy(struct wl_listener *listener, void *data)
{
...
bool
swaynag_spawn(const char *swaynag_command, struct swaynag_instance *swaynag)
{
...
void
swaynag_log(const char *swaynag_command, struct swaynag_instance *swaynag,
            const char *fmt, ...)
{
    if (!swaynag_command) {
        return;
    ...
    if (!swaynag->detailed) {
        sway_log(SWAY_ERROR,
                 "Attempting to write to non-detailed swaynag inst");
        return;
    }
    ...
    if (!str) {
        sway_log(SWAY_ERROR,
                 "Failed to allocate buffer for swaynag log entry.");
        return;
    }
...
void
swaynag_show(struct swaynag_instance *swaynag)
{
