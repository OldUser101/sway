...
static bool
is_privileged(const struct wl_global *global)
{
#if WLR_HAS_DRM_BACKEND
    if (server.drm_lease_manager != NULL) {
        struct wlr_drm_lease_device_v1 *drm_lease_dev;
        wl_list_for_each(drm_lease_dev, &server.drm_lease_manager->devices,
                         link)
        {
...
static bool
filter_global(const struct wl_client *client, const struct wl_global *global,
              void *data)
{
#if WLR_HAS_XWAYLAND
...
static void
detect_proprietary(struct wlr_backend *backend, void *data)
{
...
static void
do_renderer_recreate(void *data)
{
...
bool
server_init(struct sway_server *server)
{
    ...
    wlr_ext_output_image_capture_source_manager_v1_create(server->wl_display,
                                                          1);
...
void
server_fini(struct sway_server *server)
{
    // remove listeners
    wl_list_remove(&server->renderer_lost.link);
...
bool
server_start(struct sway_server *server)
{
#if WLR_HAS_XWAYLAND
    if (config->xwayland != XWAYLAND_MODE_DISABLED) {
        ...
        if (!server->xwayland.wlr_xwayland) {
            ...
            setenv("DISPLAY", server->xwayland.wlr_xwayland->display_name,
                   true);
...
void
server_run(struct sway_server *server)
{
