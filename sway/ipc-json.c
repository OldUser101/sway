...
static const char *
ipc_json_node_type_description(enum sway_node_type node_type)
{
    switch (node_type) {
        case N_ROOT:
            return "root";
...
static const char *
ipc_json_layout_description(enum sway_container_layout l)
{
    switch (l) {
        case L_VERT:
            return "splitv";
...
static const char *
ipc_json_orientation_description(enum sway_container_layout l)
{
    switch (l) {
        case L_VERT:
            return "vertical";
...
static const char *
ipc_json_border_description(enum sway_container_border border)
{
    switch (border) {
        case B_NONE:
            return "none";
...
ipc_json_output_transform_description(enum wl_output_transform transform)
{
    switch (transform) {
        ...
        case WL_OUTPUT_TRANSFORM_90:
            // Sway uses clockwise transforms, while WL_OUTPUT_TRANSFORM_*
...
static const char *
ipc_json_output_mode_aspect_ratio_description(
...
static json_object *
ipc_json_output_mode_description(const struct wlr_output_mode *mode)
{
    const char *pic_ar = ipc_json_output_mode_aspect_ratio_description(
...
#if WLR_HAS_XWAYLAND
static const char *
ipc_json_xwindow_type_description(struct sway_view *view)
{
...
static const char *
ipc_json_user_idle_inhibitor_description(enum sway_idle_inhibit_mode mode)
{
    switch (mode) {
...
static const char *
ipc_json_content_type_description(enum wp_content_type_v1_type type)
{
    switch (type) {
...
ipc_json_get_version(void)
{
    ...
    json_object_object_add(version, "human_readable",
                           json_object_new_string(SWAY_VERSION));
    ...
    json_object_object_add(version, "loaded_config_file_name",
...
static json_object *
ipc_json_create_node(int id, const char *type, char *name, bool focused,
                     json_object *focus, struct wlr_box *box)
{
...
static void
ipc_json_describe_wlr_output(struct wlr_output *wlr_output, json_object *object)
{
...
static void
ipc_json_describe_output(struct sway_output *output, json_object *object)
{
    ipc_json_describe_wlr_output(output->wlr_output, object);
}

static void
ipc_json_describe_enabled_output(struct sway_output *output,
                                 json_object *object)
{
    ...
    if (parent_box.width != 0 && parent_box.height != 0) {
        ...
        json_object_object_add(object, "percent",
                               json_object_new_double(percent));
    }
...
json_object *
ipc_json_describe_disabled_output(struct sway_output *output)
{
    struct wlr_output *wlr_output = output->wlr_output;
...
json_object *
ipc_json_describe_non_desktop_output(struct sway_output_non_desktop *output)
{
    struct wlr_output *wlr_output = output->wlr_output;
...
ipc_json_describe_scratchpad_output(void)
{
    ...
    json_object *workspace =
        ipc_json_create_node(i3_scratch_id, "workspace", "__i3_scratch", false,
                             workspace_focus, &box);
    ...
    json_object *output = ipc_json_create_node(i3_output_id, "output", "__i3",
                                               false, output_focus, &box);
...
static void
ipc_json_describe_workspace(struct sway_workspace *workspace,
                            json_object *object)
{
    json_object_object_add(object, "num", json_object_new_int(workspace->num));
    ...
    json_object_object_add(
        object, "output",
        workspace->output
            ? json_object_new_string(workspace->output->wlr_output->name)
            : NULL);
    ...
    json_object_object_add(
        ...
        workspace->representation
            ? json_object_new_string(workspace->representation)
            : NULL);

    json_object_object_add(
        object, "layout",
        json_object_new_string(ipc_json_layout_description(workspace->layout)));
    json_object_object_add(
        object, "orientation",
        json_object_new_string(
...
static void
get_deco_rect(struct sway_container *c, struct wlr_box *deco_rect)
{
    enum sway_container_layout parent_layout = container_parent_layout(c);
    list_t *siblings = container_get_siblings(c);
    bool tab_or_stack =
        (parent_layout == L_TABBED || parent_layout == L_STACKED) &&
    ...
    if (!container_is_floating(c)) {
        if (parent_layout == L_TABBED) {
            deco_rect->width =
                c->pending.parent
                    ? c->pending.parent->pending.width /
                          c->pending.parent->pending.children->length
                    : c->pending.workspace->width /
                          c->pending.workspace->tiling->length;
...
static void
ipc_json_describe_view(struct sway_container *c, json_object *object)
{
    ...
    json_object_object_add(
        object, "foreign_toplevel_identifier",
        c->view->ext_foreign_toplevel
            ? json_object_new_string(c->view->ext_foreign_toplevel->identifier)
            : NULL);
    ...
    json_object_object_add(object, "window_rect",
                           ipc_json_create_rect(&window_box));

    struct wlr_box geometry = {0, 0, c->view->natural_width,
                               c->view->natural_height};
    json_object_object_add(object, "geometry", ipc_json_create_rect(&geometry));

    json_object_object_add(object, "max_render_time",
                           json_object_new_int(c->view->max_render_time));

    json_object_object_add(object, "allow_tearing",
                           json_object_new_boolean(view_can_tear(c->view)));

    json_object_object_add(object, "shell",
                           json_object_new_string(view_get_shell(c->view)));
    ...
    json_object_object_add(object, "sandbox_instance_id",
                           sandbox_instance_id
                               ? json_object_new_string(sandbox_instance_id)
                               : NULL);
    ...
    if (user_inhibitor) {
        json_object_object_add(
            ...
            json_object_new_string(ipc_json_user_idle_inhibitor_description(
                user_inhibitor->mode)));
    } else {
    ...
    if (c->view->surface != NULL) {
        content_type = wlr_surface_get_content_type_v1(
            server.content_type_manager_v1, c->view->surface);
    }
    if (content_type != WP_CONTENT_TYPE_V1_TYPE_NONE) {
        json_object_object_add(
            object, "content_type",
            json_object_new_string(
                ipc_json_content_type_description(content_type)));
    }
    ...
    if (c->view->type == SWAY_VIEW_XWAYLAND) {
        ...
        json_object_object_add(window_props, "transient_for",
                               parent_id ? json_object_new_int(parent_id)
                                         : NULL);
...
static void
ipc_json_describe_container(struct sway_container *c, json_object *object)
{
    json_object_object_add(object, "name",
    ...
    json_object_object_add(
        object, "layout",
        json_object_new_string(ipc_json_layout_description(c->pending.layout)));
    ...
    bool urgent =
        c->view ? view_is_urgent(c->view) : container_has_urgent_child(c);
    json_object_object_add(object, "urgent", json_object_new_boolean(urgent));
    json_object_object_add(object, "sticky",
                           json_object_new_boolean(c->is_sticky));

    // sway doesn't track the floating reason, so we can't use "auto_on" or
    // "user_off"
    json_object_object_add(
        object, "floating",
    ...
    // sway doesn't track if window was resized in scratchpad, so we can't use
    // "changed"
    json_object_object_add(
    ...
    json_object_object_add(
        object, "border",
        json_object_new_string(ipc_json_border_description(c->current.border)));
    json_object_object_add(object, "current_border_width",
...
static void
focus_inactive_children_iterator(struct sway_node *node, void *_data)
{
    ...
    if (data->node == &root->node) {
        ...
        for (int i = 0; i < len; ++i) {
            if ((size_t)json_object_get_int(
                    json_object_array_get_idx(focus, i)) == id) {
                return;
            }
        }
        node = &output->node;
...
json_object *
ipc_json_describe_node(struct sway_node *node)
{
    ...
    json_object *object = ipc_json_create_node(
        (int)node->id, ipc_json_node_type_description(node->type), name,
        focused, focus, &box);
...
json_object *
ipc_json_describe_node_recursive(struct sway_node *node)
{
        ...
        case N_OUTPUT:
            for (i = 0; i < node->sway_output->workspaces->length; ++i) {
                struct sway_workspace *ws =
                    node->sway_output->workspaces->items[i];
                json_object_array_add(
                    children, ipc_json_describe_node_recursive(&ws->node));
            }
            break;
        case N_WORKSPACE:
            for (i = 0; i < node->sway_workspace->tiling->length; ++i) {
                struct sway_container *con =
                    node->sway_workspace->tiling->items[i];
                json_object_array_add(
                    children, ipc_json_describe_node_recursive(&con->node));
            }
            break;
        case N_CONTAINER:
            if (node->sway_container->pending.children) {
                for (i = 0; i < node->sway_container->pending.children->length;
                     ++i) {
...
describe_libinput_device(struct libinput_device *device)
{
    ...
    if ((click_methods & ~LIBINPUT_CONFIG_CLICK_METHOD_NONE) != 0) {
        ...
        const char *button_map = "unknown";
        switch (
            libinput_device_config_click_get_clickfinger_button_map(device)) {
...
json_object *
ipc_json_describe_input(struct sway_input_device *device)
{
...
json_object *
ipc_json_describe_seat(struct sway_seat *seat)
{
    ...
    {
        json_object_array_add(devices,
                              ipc_json_describe_input(device->input_device));
    }
