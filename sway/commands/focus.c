...
static bool
get_direction_from_next_prev(struct sway_container *container,
                             struct sway_seat *seat, const char *name,
...
get_node_in_output_direction(struct sway_output *output, enum wlr_direction dir)
{
...
static struct sway_node *
node_get_in_direction_tiling(struct sway_container *container,
                             ...
                             bool descend)
{
            ...
            if (desired < 0 || desired >= siblings->length) {
                ...
                if (!descend) {
                    ...
                    struct sway_container *c =
                        seat_get_focus_inactive_view(seat, &desired_con->node);
                    return &c->node;
                }
            }
        }
    ...
    if ((config->focus_wrapping != WRAP_WORKSPACE ||
         container->node.type == N_WORKSPACE) &&
        new_output) {
        return get_node_in_output_direction(new_output, dir);
    }
    ...
    if (wrap_candidate) {
        struct sway_container *wrap_inactive =
            seat_get_focus_inactive_view(seat, &wrap_candidate->node);
...
static struct sway_node *
node_get_in_direction_floating(struct sway_container *con,
                               struct sway_seat *seat, enum wlr_direction dir)
{
    ...
    for (int i = 0; i < con->pending.workspace->floating->length; i++) {
        struct sway_container *floater =
            con->pending.workspace->floating->items[i];
        if (floater == con) {
            continue;
        }
        float distance =
            dir == WLR_DIRECTION_LEFT || dir == WLR_DIRECTION_RIGHT
...
focus_mode(struct sway_workspace *ws, struct sway_seat *seat, bool floating)
{
...
static struct cmd_results *
focus_output(struct sway_seat *seat, int argc, char **argv)
{
    if (!argc) {
    ...
    if (!output) {
        ...
        if (!ws) {
            ...
            return cmd_results_new(
                CMD_FAILURE, "No focused workspace to base directions off of.");
        }
...
cmd_focus(int argc, char **argv)
{
    ...
    if (argc == 0) {
        if (!container) {
            return cmd_results_new(CMD_FAILURE,
                                   "No container to focus was specified.");
        }

        if (container_is_scratchpad_hidden_or_child(container)) {
            root_scratchpad_show(container);
        }
        // if we are switching to a container under a fullscreen window, we
        // first need to exit fullscreen so that the newly focused container
        // becomes visible
    ...
    if (!parse_direction(argv[0], &direction)) {
        if (!get_direction_from_next_prev(container, seat, argv[0],
                                          &direction)) {
            return cmd_results_new(CMD_INVALID,
                                   "Expected 'focus "
                                   "<direction|next|prev|parent|child|mode_"
    ...
    if (container_is_floating(container) &&
        ...
        next_focus =
            node_get_in_direction_tiling(container, seat, direction, descend);
    }
    if (next_focus) {
