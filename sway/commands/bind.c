...
void
free_sway_binding(struct sway_binding *binding)
{
    if (!binding) {
        return;
    }
...
void
free_switch_binding(struct sway_switch_binding *binding)
{
    if (!binding) {
        return;
    }
 ...
 */
static bool
binding_switch_compare(struct sway_switch_binding *binding_a,
 ...
 */
static bool
binding_key_compare(struct sway_binding *binding_a,
                    struct sway_binding *binding_b)
{
    ...
    uint32_t conflict_generating_flags = BINDING_RELEASE | BINDING_BORDER |
                                         ...
                                         BINDING_LOCKED | BINDING_INHIBITED;
...
identify_key(const char *name, bool first_key, uint32_t *key_val,
    ...
    if (*type == BINDING_MOUSECODE) {
        ...
        if (!button) {
            if (message) {
                ...
                return cmd_results_new(CMD_INVALID, "Unknown button code %s",
                                       name);
            }
        }
        *key_val = button;
        ...
        *key_val = button;
        ...
        if (!xkb_keycode_is_legal_ext(keycode)) {
            if (first_key) {
                ...
                return cmd_results_new(CMD_INVALID, "Invalid keycode '%s'",
                                       name);
            }
        }
        *key_val = keycode;
...
static struct cmd_results *
switch_binding_add(struct sway_switch_binding *binding, const char *bindtype,
                   const char *switchcombo, bool warn)
{
    ...
    if (!overwritten) {
        ...
        sway_log(SWAY_DEBUG, "%s - Bound %s to command `%s`", bindtype,
                 switchcombo, binding->command);
    }
...
static struct cmd_results *
switch_binding_remove(struct sway_switch_binding *binding, const char *bindtype,
                      const char *switchcombo)
{
    ...
    for (int i = 0; i < mode_bindings->length; ++i) {
        ...
        if (binding_switch_compare(binding, config_binding)) {
            ...
            sway_log(SWAY_DEBUG, "%s - Unbound %s switch", bindtype,
                     switchcombo);
            return cmd_results_new(CMD_SUCCESS, NULL);
        }
    }
...
binding_upsert(struct sway_binding *binding, list_t *mode_bindings)
{
    for (int i = 0; i < mode_bindings->length; ++i) {
...
static struct cmd_results *
binding_add(struct sway_binding *binding, list_t *mode_bindings,
...
static struct cmd_results *
binding_remove(struct sway_binding *binding, list_t *mode_bindings,
               const char *bindtype, const char *keycombo)
{
    for (int i = 0; i < mode_bindings->length; ++i) {
        struct sway_binding *config_binding = mode_bindings->items[i];
        if (binding_key_compare(binding, config_binding)) {
            sway_log(SWAY_DEBUG, "%s - Unbound `%s` from device '%s'", bindtype,
                     keycombo, binding->input);
            free_sway_binding(config_binding);
...
cmd_bindsym_or_bindcode(int argc, char **argv, bool bindcode, bool unbind)
{
    ...
    if (!translate_binding(binding)) {
        sway_log(SWAY_INFO, "Unable to translate bindsym into bindcode: %s",
                 argv[0]);
    }
...
cmd_bind_or_unbind_switch(int argc, char **argv, bool unbind)
{
    ...
    struct sway_switch_binding *binding =
        calloc(1, sizeof(struct sway_switch_binding));
    if (!binding) {
        return cmd_results_new(CMD_FAILURE, "Unable to allocate binding");
    }
    ...
    if (strcmp(split->items[0], "tablet") == 0) {
        ...
        return cmd_results_new(CMD_FAILURE,
                               ...
                               "unknown switch %s)",
                               bindtype, (const char *)split->items[0]);
    }
 ...
 */
void
seat_execute_command(struct sway_seat *seat, struct sway_binding *binding)
{
    if (!config->active) {
    ...
    if (binding->type == BINDING_MOUSESYM ||
        ...
        struct sway_node *node =
            node_at_coords(seat, seat->cursor->cursor->x,
                           seat->cursor->cursor->y, &surface, &sx, &sy);
...
bool
translate_binding(struct sway_binding *binding)
{
...
void
binding_add_translated(struct sway_binding *binding, list_t *mode_bindings)
{
    struct sway_binding *config_binding =
        binding_upsert(binding, mode_bindings);
