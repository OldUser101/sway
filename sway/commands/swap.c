...
static bool
test_con_id(struct sway_container *container, void *data)
{
    size_t *con_id = data;
...
#if WLR_HAS_XWAYLAND
static bool
test_id(struct sway_container *container, void *data)
{
...
static bool
test_mark(struct sway_container *container, void *mark)
{
...
cmd_swap(int argc, char **argv)
{
    ...
    if (!other) {
        error = cmd_results_new(CMD_FAILURE, "Failed to find %s '%s'", argv[2],
                                value);
    } else if (!current) {
        ...
        error =
            cmd_results_new(CMD_FAILURE, "Cannot swap a container with itself");
    } else if (container_has_ancestor(current, other) ||
               container_has_ancestor(other, current)) {
        error =
            cmd_results_new(CMD_FAILURE, "Cannot swap ancestor and descendant");
    }
