...
static const char expected_syntax[] =
    "Expected 'tag workspace <name> set|clear <tag>' or "
...
struct cmd_results *cmd_tag(int argc, char **argv) {
    ...
    if (strcasecmp(argv[1], "set") == 0) {
	    ...
	    // 'tag workspace <name> set|clear <tag>'
        int end = argn;
        while (end < argc && (strcasecmp(argv[end], "set") != 0 && strcasecmp(argv[end], "clear") != 0)) {
            ++end;
        }
        char *name = join_args(argv + argn, end - argn);
        workspace = workspace_by_name(name);
        free(name);
        argn = end;
    }
    ...
    if (tag_op == SWAY_TAG_SET) {
        sway_log(SWAY_DEBUG, "adding tag '%s' to workspace '%s'", tag, workspace->name);
    } else {
        sway_log(SWAY_DEBUG, "removing tag '%s' from workspace '%s'", tag, workspace->name);
    }
