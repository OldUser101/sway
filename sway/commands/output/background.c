...
output_cmd_background(int argc, char **argv)
{
    ...
    if (strcasecmp(argv[1], "solid_color") == 0) {
        ...
        if (!(output->background = strdup(argv[0])))
            goto cleanup;
        if (!(output->background_option = strdup("solid_color")))
            goto cleanup;
        ...
        if (config->reading && *src != '/') {
            ...
            snprintf(real_src, strlen(conf_path) + strlen(src) + 2, "%s/%s",
                     conf_path, src);
            free(src);
            free(conf);
            src = real_src;
        ...
        if (argc && *argv[0] == '#') {
            if (validate_color(argv[0])) {
                if (!(fallback = strdup(argv[0])))
                    goto cleanup;
                output->background_fallback = fallback;
            } else {
                sway_log(SWAY_ERROR,
                         "fallback '%s' should be of the form #RRGGBB",
                         argv[0]);
        ...
        if (!can_access) {
            if (!fallback) {
                sway_log(SWAY_ERROR,
                         "Unable to access background file '%s' "
                         "and no valid fallback provided",
                         src);
                struct cmd_results *res = cmd_results_new(
                    CMD_FAILURE,
                    "Unable to access "
                    "background file '%s' and no valid fallback provided",
                    src);
                free(src);
                return res;
            }
            sway_log(SWAY_DEBUG, "Cannot access file '%s', using fallback '%s'",
                     src, fallback);
            ...
            if (!(output->background_option = strdup(mode)))
                goto cleanup;
        }
    }
