...
static bool
read_file_into_buf(const char *path, void **buf, size_t *size)
{
    /* Why not use fopen/fread directly? glibc will succesfully open
     * directories, not just files, and supports seeking on them. Instead, we
     * directly work with file descriptors and use the more consistent
     * open/fstat/read. */
...
struct cmd_results *
output_cmd_color_profile(int argc, char **argv)
{
    ...
    if (!argc) {
        return cmd_results_new(CMD_INVALID,
                               "Missing color_profile first argument.");
    }
    ...
    if (strcmp(*argv, "srgb") == 0) {
        ...
        if (argc < 2) {
            return cmd_results_new(CMD_INVALID,
                                   "Invalid color profile specification: icc "
                                   "type requires a file");
        }

        char *icc_path = strdup(argv[1]);
        if (!expand_path(&icc_path)) {
            struct cmd_results *cmd_res = cmd_results_new(
                CMD_INVALID,
        ...
        if (!tmp) {
            ...
            return cmd_results_new(CMD_FAILURE,
                                   "Failed to load color profile: failed to "
                                   "initialize transform from ICC");
        }
        free(data);
        ...
        return cmd_results_new(CMD_INVALID,
                               "Invalid color profile specification: first "
                               "argument should be icc|srgb");
    }
