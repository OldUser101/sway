...
container_find_resize_parent(struct sway_container *con, uint32_t axis)
{
...
void
container_resize_tiled(struct sway_container *con, uint32_t axis, int amount)
{
    if (!con) {
        return;
    }
    ...
    if (is_horizontal(axis)) {
        ...
        for (int i = 0; i < siblings->length; ++i) {
            struct sway_container *con = siblings->items[i];
            con->height_fraction =
                con->pending.height / con->child_total_height;
        }
    ...
    if (amount->unit == MOVEMENT_UNIT_PPT) {
        ...
        if (is_horizontal(axis)) {
            ...
            if (parent) {
                ...
                amount->amount =
                    (float)current->pending.workspace->height * pct;
            }
        }
    }
 ...
 */
static struct cmd_results *
resize_set_tiled(struct sway_container *con, struct movement_amount *width,
    ...
    if (width->amount) {
        if (width->unit == MOVEMENT_UNIT_PPT ||
            ...
            if (parent) {
                ...
                width->amount =
                    con->pending.workspace->width * width->amount / 100;
            }
    ...
    if (height->amount) {
        if (height->unit == MOVEMENT_UNIT_PPT ||
            ...
            if (parent) {
                ...
                height->amount =
                    con->pending.workspace->height * height->amount / 100;
            }
 ...
 */
static struct cmd_results *
resize_set_floating(struct sway_container *con, struct movement_amount *width,
                    struct movement_amount *height)
{
    ...
    if (height->amount) {
        switch (height->unit) {
            ...
            case MOVEMENT_UNIT_DEFAULT:
                height->amount =
                    fmax(min_height, fmin(height->amount, max_height));
...
cmd_resize_adjust(int argc, char **argv, int multiplier)
{
    ...
    if (container_is_floating(con)) {
        ...
        if (first_amount.unit == MOVEMENT_UNIT_PX) {
            ...
            return cmd_results_new(
                CMD_INVALID, "Floating containers cannot use ppt measurements");
        }
    }
