...
static const char expected_syntax[] =
    ...
    "'move [--no-auto-back-and-forth] <container|window> [to] workspace "
    "<name>' or "
    "'move <container|window|workspace> [to] output <name|direction>' or "
    "'move <container|window> [to] mark <mark>'";

static struct sway_output *
output_in_direction(const char *direction_string, struct sway_output *reference,
                    int ref_lx, int ref_ly)
{
...
static bool
is_parallel(enum sway_container_layout layout, enum wlr_direction dir)
{
    switch (layout) {
        case L_TABBED:
        case L_HORIZ:
 ...
 */
static void
workspace_focus_fullscreen(struct sway_workspace *workspace)
{
...
static void
container_move_to_container_from_direction(struct sway_container *container,
                                           ...
                                           enum wlr_direction move_dir)
{
    if (destination->view) {
        if (destination->pending.parent == container->pending.parent &&
            ...
            if (destination->pending.parent) {
                container_insert_child(destination->pending.parent, container,
                                       index);
            } else {
    ...
    if (is_parallel(destination->pending.layout, move_dir)) {
        ...
        int index =
            move_dir == WLR_DIRECTION_RIGHT || move_dir == WLR_DIRECTION_DOWN
                ? 0
                : destination->pending.children->length;
    ...
    container_move_to_container_from_direction(
        container, focus_inactive->sway_container, move_dir);
}

static void
container_move_to_workspace_from_direction(struct sway_container *container,
                                           ...
                                           enum wlr_direction move_dir)
{
    ...
    sway_log(SWAY_DEBUG, "Reparenting container (perpendicular)");
    struct sway_container *focus_inactive =
        seat_get_focus_inactive_tiling(config->handler_context.seat, workspace);
    if (!focus_inactive) {
...
static void
container_move_to_workspace(struct sway_container *container,
                            struct sway_workspace *workspace)
{
    ...
    if (container_is_floating(container)) {
        ...
        if (old_output != workspace->output &&
            !container->pending.fullscreen_mode) {
            ...
            floating_fix_coordinates(container, &old_workspace_box,
                                     &workspace_box);
...
static void
container_move_to_container(struct sway_container *container,
                            struct sway_container *destination)
{
...
static bool
container_move_to_next_output(struct sway_container *container,
                              struct sway_output *output,
                              enum wlr_direction move_dir)
{
    struct sway_output *next_output = output_get_in_direction(output, move_dir);
    if (next_output) {
        ...
        switch (container->pending.fullscreen_mode) {
            case FULLSCREEN_NONE:
                container_move_to_workspace_from_direction(container, ws,
                                                           move_dir);
                return true;
...
static bool
container_move_in_direction(struct sway_container *container,
                            enum wlr_direction move_dir)
{
    ...
    switch (container->pending.fullscreen_mode) {
        ...
        case FULLSCREEN_WORKSPACE:
            return container_move_to_next_output(
                container, container->pending.workspace->output, move_dir);
        case FULLSCREEN_GLOBAL:
            return false;
    }
    ...
    while (!ancestor) {
        ...
        if (current->pending.fullscreen_mode ||
            container_is_floating(current)) {
            return false;
        }
        ...
        if (current == container) {
            if (target) {
                ...
                container_move_to_container_from_direction(container, target,
                                                           move_dir);
                return true;
                ...
                return container_move_to_next_output(
                    container, current->pending.workspace->output, move_dir);
            } else {
    ...
    if (target) {
        ...
        container_move_to_container_from_direction(container, target, move_dir);
        return true;
        ...
        return container_move_to_next_output(
            container, ancestor->pending.workspace->output, move_dir);
    } else {
...
cmd_move_container(bool no_auto_back_and_forth, int argc, char **argv)
{
    struct cmd_results *error = NULL;
    if ((error =
             checkarg(argc, "move container/window", EXPECTED_AT_LEAST, 2))) {
        return error;
    }
    ...
    if (strcasecmp(argv[0], "workspace") == 0) {
        ...
        if (strcasecmp(argv[1], "next") == 0 ||
            ...
            ws = workspace_by_desc(argv[1]);
        } else if (strcasecmp(argv[1], "back_and_forth") == 0) {
            if (!(ws = workspace_by_desc(argv[1]))) {
                if (seat->prev_workspace > -1) {
                    ws_num = seat->prev_workspace;
                } else {
                    return cmd_results_new(
                        CMD_FAILURE, "No workspace was previously active.");
                }
            }
        } else {
            // move [window|container] [to] "workspace x"
            if (argc < 2) {
                return cmd_results_new(CMD_INVALID, "%s", expected_syntax);
            }

            if ((ws_num = workspace_read_num(argv[1])) == -1) {
                return cmd_results_new(CMD_INVALID,
                                       "Invalid workspace number: %s", argv[1]);
            }

            ws = workspace_by_number(ws_num);

            if (!no_auto_back_and_forth && config->auto_back_and_forth &&
                seat->prev_workspace > -1) {
                // auto back and forth move
                if (old_ws && old_ws->num > -1 && old_ws->num == ws_num) {
        ...
        if (!ws) {
            ...
            if (container_is_sticky_or_child(container)) {
                struct sway_output *new_output =
                    workspace_get_initial_output(ws_num);
        ...
        struct sway_output *new_output = output_in_direction(
            argv[1], old_output, container->pending.x, container->pending.y);
        if (!new_output) {
            return cmd_results_new(CMD_FAILURE,
                                   "Can't find output with name/direction '%s'",
                                   argv[1]);
        }
        ...
        if (dest_con == NULL) {
            return cmd_results_new(CMD_FAILURE, "Mark '%s' not found", argv[1]);
        }
...
static void
workspace_move_to_output(struct sway_workspace *workspace,
                         struct sway_output *output)
{
    ...
    if (!sway_assert(new_output_old_ws,
                     "Expected output to have a workspace")) {
        return;
    }
    ...
    if (old_output->workspaces->length == 0) {
        int ws_num = workspace_next_num(old_output->wlr_output->name);
        struct sway_workspace *ws = workspace_create(old_output, ws_num);
...
static struct cmd_results *
cmd_move_workspace(int argc, char **argv)
{
    struct cmd_results *error = NULL;
    ...
    if (!argc) {
        return cmd_results_new(
            CMD_INVALID, "Expected 'move workspace to [output] <output>'");
    }
    ...
    struct sway_output *new_output =
        output_in_direction(argv[0], old_output, center_x, center_y);
    if (!new_output) {
        return cmd_results_new(
            CMD_FAILURE, "Can't find output with name/direction '%s'", argv[0]);
    }
...
static struct cmd_results *
cmd_move_to_position_pointer(struct sway_container *container)
{
    struct sway_seat *seat = config->handler_context.seat;
    if (!seat->cursor) {
...
cmd_move_to_scratchpad(void)
{
    ...
    if (node->type == N_WORKSPACE && ws->tiling->length == 0) {
        return cmd_results_new(
            CMD_INVALID, "Can't move an empty workspace to the scratchpad");
    }
    if (node->type == N_WORKSPACE) {
