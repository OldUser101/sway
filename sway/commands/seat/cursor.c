...
handle_command(struct sway_cursor *cursor, int argc, char **argv)
{
...
seat_cmd_cursor(int argc, char **argv)
{
    ...
    if (strcmp(sc->name, "*") != 0) {
        ...
        if (!seat) {
            return cmd_results_new(CMD_FAILURE, "Seat %s does not exist",
                                   sc->name);
        }
...
static struct cmd_results *
press_or_release(struct sway_cursor *cursor, char *action, char *button_str)
{
    ...
    if (message) {
        struct cmd_results *error = cmd_results_new(CMD_INVALID, "%s", message);
        free(message);
        return error;
        ...
        double delta =
            (button == SWAY_SCROLL_UP || button == SWAY_SCROLL_LEFT) ? -1 : 1;
        struct wlr_pointer_axis_event event = {.pointer = NULL,
                                               .time_msec = 0,
                                               .source =
