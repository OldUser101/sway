...
static void
toggle_supported_send_events_for_device(struct input_config *ic,
...
static void
toggle_select_send_events_for_device(struct input_config *ic,
                                     struct sway_input_device *input_device,
                                     int argc, char **argv)
{
#if WLR_HAS_LIBINPUT_BACKEND
    ...
    if (strcasecmp(argv[0], "enabled") == 0) {
        ...
        for (int i = 1; i < argc; ++i) {
            if (mode_for_name(argv[i]) == -1) {
                return cmd_results_new(CMD_INVALID, "Invalid toggle mode %s",
                                       argv[i]);
            }
        }
