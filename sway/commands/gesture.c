...
void
free_gesture_binding(struct sway_gesture_binding *binding)
{
    if (!binding) {
        return;
    }
 ...
 */
static bool
binding_gesture_equal(struct sway_gesture_binding *binding_a,
...
gesture_binding_add(struct sway_gesture_binding *binding,
 ...
 */
static struct cmd_results *
gesture_binding_remove(struct sway_gesture_binding *binding,
                       const char *gesturecombo)
{
    list_t *mode_bindings = config->current_mode->gesture_bindings;
    for (int i = 0; i < mode_bindings->length; ++i) {
        struct sway_gesture_binding *config_binding = mode_bindings->items[i];
 ...
 */
static struct cmd_results *
cmd_bind_or_unbind_gesture(int argc, char **argv, bool unbind)
{
    int minargs = 2;
    ...
    struct sway_gesture_binding *binding =
        calloc(1, sizeof(struct sway_gesture_binding));
    if (!binding) {
...
struct cmd_results *
cmd_bindgesture(int argc, char **argv)
{
    return cmd_bind_or_unbind_gesture(argc, argv, false);
}

struct cmd_results *
cmd_unbindgesture(int argc, char **argv)
{
