...
static void
container_update_iterator(struct sway_container *con, void *data)
{
    container_update(con);
}

static struct cmd_results *
handle_command(int argc, char **argv, char *cmd_name,
               struct border_colors *class, const char *default_indicator)
{
    ...
    struct {
        ...
        const char *hex;
        float *rgba[4];
    } properties[] = {{"border", argv[0], colors.border},
...
cmd_client_focused_tab_title(int argc, char **argv)
{
    struct cmd_results *result =
        handle_command(argc, argv, "client.focused_tab_title",
