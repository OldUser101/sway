...
static void
set_border(struct sway_container *con, enum sway_container_border new_border)
{
    if (con->view) {
...
static void
border_toggle(struct sway_container *con)
{
...
cmd_border(int argc, char **argv)
{
    ...
    if (strcmp(argv[0], "none") == 0) {
        ...
        return cmd_results_new(
            CMD_INVALID, "Expected 'border <none|normal|pixel|csd|toggle>' "
                         "or 'border pixel <px>'");
    }
    if (argc == 2) {
