 ...
 */
static bool
state_erase_key(struct sway_shortcut_state *state, uint32_t keycode)
{
    bool found = false;
    size_t j = 0;
 ...
 */
static void
state_add_key(struct sway_shortcut_state *state, uint32_t keycode,
              uint32_t key_id)
{
 ...
 */
static bool
update_shortcut_state(struct sway_shortcut_state *state, uint32_t keycode,
 ...
 */
static void
get_active_binding(const struct sway_shortcut_state *state, list_t *bindings,
                   struct sway_binding **current_binding, uint32_t modifiers,
                   bool release, bool locked, bool inhibited, const char *input,
 ...
 */
static bool
keyboard_execute_compositor_binding(struct sway_keyboard *keyboard,
...
static bool
keyboard_execute_pointer_keysyms(struct sway_keyboard *keyboard, uint32_t time,
                                 const xkb_keysym_t *pressed_keysyms,
                                 size_t keysyms_len,
 ...
 */
static size_t
keyboard_keysyms_translated(struct sway_keyboard *keyboard,
 ...
 */
static size_t
keyboard_keysyms_raw(struct sway_keyboard *keyboard, xkb_keycode_t keycode,
                     const xkb_keysym_t **keysyms, uint32_t *modifiers)
{
    *modifiers = wlr_keyboard_get_modifiers(keyboard->wlr);
    ...
    return xkb_keymap_key_get_syms_by_level(keyboard->wlr->keymap, keycode,
                                            layout_index, 0, keysyms);
}

void
sway_keyboard_disarm_key_repeat(struct sway_keyboard *keyboard)
{
    if (!keyboard) {
        return;
    }
...
static void
update_keyboard_state(struct sway_keyboard *keyboard, uint32_t raw_keycode,
                      ...
                      struct key_info *keyinfo)
{
    ...
    keyinfo->raw_keysyms_len =
        keyboard_keysyms_raw(keyboard, keyinfo->keycode, &keyinfo->raw_keysyms,
    ...
    for (size_t i = 0; i < keyinfo->translated_keysyms_len; ++i) {
        update_shortcut_state(&keyboard->state_keysyms_translated, raw_keycode,
                              keystate, keyinfo->translated_keysyms[i],
                              keyinfo->code_modifiers);
    }
}
...
keyboard_get_im_grab(struct sway_keyboard *keyboard)
{
    struct wlr_input_method_v2 *input_method =
        keyboard->seat_device->sway_seat->im_relay.input_method;
...
static void
handle_key_event(struct sway_keyboard *keyboard,
                 struct wlr_keyboard_key_event *event)
{
    ...
    bool shortcuts_inhibited =
        sway_inhibitor && sway_inhibitor->inhibitor->active;
    ...
    get_active_binding(&keyboard->state_keycodes,
                       ...
                       &binding_released, keyinfo.code_modifiers, true, locked,
    ...
    get_active_binding(&keyboard->state_keysyms_raw,
                       ...
                       keyinfo.raw_modifiers, true, locked, shortcuts_inhibited,
    ...
    if (event->state == WL_KEYBOARD_KEY_STATE_PRESSED) {
        ...
        get_active_binding(
            ...
            &binding, keyinfo.raw_modifiers, false, locked, shortcuts_inhibited,
    ...
    if (binding && !(binding->flags & BINDING_NOREPEAT) &&
        ...
        if (wl_event_source_timer_update(keyboard->key_repeat_source,
                                         keyboard->wlr->repeat_info.delay) <
            0) {
    ...
    if (!handled) {
        handled = keyboard_execute_pointer_keysyms(
            keyboard, event->time_msec, keyinfo.translated_keysyms,
            keyinfo.translated_keysyms_len, event->state);
    }
    ...
    if (event->state == WL_KEYBOARD_KEY_STATE_RELEASED) {
        ...
        bool pressed_sent =
            update_shortcut_state(&keyboard->state_pressed_sent, event->keycode,
...
handle_keyboard_group_enter(struct wl_listener *listener, void *data)
{
    ...
    {
        struct key_info keyinfo;
        update_keyboard_state(keyboard, *keycode, WL_KEYBOARD_KEY_STATE_PRESSED,
                              &keyinfo);
    }
}
...
static void
handle_modifier_event(struct sway_keyboard *keyboard)
{
    if (!keyboard->wlr->group) {
        ...
        if (kb_grab) {
            ...
            struct wlr_seat *wlr_seat =
                keyboard->seat_device->sway_seat->wlr_seat;
            wlr_seat_set_keyboard(wlr_seat, keyboard->wlr);
...
sway_keyboard_create(struct sway_seat *seat, struct sway_seat_device *device)
{
    struct sway_keyboard *keyboard = calloc(1, sizeof(struct sway_keyboard));
...
static void
handle_xkb_context_log(struct xkb_context *context, enum xkb_log_level level,
...
struct xkb_keymap *
sway_keyboard_compile_keymap(struct input_config *ic, char **error)
{
    ...
    if (ic && ic->xkb_file) {
        ...
        if (!keymap_file) {
            ...
            if (error) {
                *error = format_str("cannot read xkb file %s: %s", ic->xkb_file,
                                    strerror(errno));
            }
            goto cleanup;
        }
...
static bool
repeat_info_match(struct sway_keyboard *a, struct wlr_keyboard *b)
{
...
static void
sway_keyboard_group_remove(struct sway_keyboard *keyboard)
{
    ...
    if (wl_list_empty(&wlr_group->devices)) {
        sway_log(SWAY_DEBUG, "Destroying empty keyboard group %p", wlr_group);
...
static void
sway_keyboard_group_remove_invalid(struct sway_keyboard *keyboard)
{
    if (!keyboard->wlr->group) {
        return;
    }
...
static void
sway_keyboard_group_add(struct sway_keyboard *keyboard)
{
    struct sway_input_device *device = keyboard->seat_device->input_device;
    struct sway_seat *seat = keyboard->seat_device->sway_seat;
    struct seat_config *sc = seat_get_config(seat);
    ...
    sway_log(SWAY_DEBUG, "Adding keyboard %s to group %p", device->identifier,
             sway_group->wlr_group);
...
static void
sway_keyboard_set_layout(struct sway_keyboard *keyboard,
                         struct input_config *input_config)
{
    struct xkb_keymap *keymap =
        sway_keyboard_compile_keymap(input_config, NULL);
    if (!keymap) {
    ...
    bool keymap_changed =
        keyboard->keymap ? !wlr_keyboard_keymaps_match(keyboard->keymap, keymap)
                         : true;
...
void
sway_keyboard_configure(struct sway_keyboard *keyboard)
{
    struct input_config *input_config =
    ...
    if (repeat_info_changed || config->reloading) {
        ...
        wlr_keyboard_set_repeat_info(keyboard->wlr, keyboard->repeat_rate,
...
void
sway_keyboard_destroy(struct sway_keyboard *keyboard)
{
    if (!keyboard) {
        return;
    }
