...
static void
attach_tablet_pad(struct sway_tablet_pad *tablet_pad,
...
struct sway_tablet *
sway_tablet_create(struct sway_seat *seat, struct sway_seat_device *device)
{
    struct sway_tablet *tablet = calloc(1, sizeof(struct sway_tablet));
...
void
sway_configure_tablet(struct sway_tablet *tablet)
{
...
void
sway_tablet_destroy(struct sway_tablet *tablet)
{
    if (!tablet) {
        return;
    }
...
{
    struct sway_tablet_tool *tool = wl_container_of(listener, tool, set_cursor);
...
void
sway_tablet_tool_configure(struct sway_tablet *tablet,
                           struct wlr_tablet_tool *wlr_tool)
{
    struct sway_tablet_tool *tool = calloc(1, sizeof(struct sway_tablet_tool));
    ...
    tool->tablet_v2_tool = wlr_tablet_tool_create(
        server.tablet_v2, tablet->seat_device->sway_seat->wlr_seat, wlr_tool);
...
sway_tablet_pad_create(struct sway_seat *seat, struct sway_seat_device *device)
{
    struct sway_tablet_pad *tablet_pad =
    ...
    tablet_pad->wlr =
        wlr_tablet_pad_from_input_device(device->input_device->wlr_device);
...
void
sway_configure_tablet_pad(struct sway_tablet_pad *tablet_pad)
{
    ...
    wl_signal_add(&tablet_pad->wlr->events.attach_tablet, &tablet_pad->attach);
...
void
sway_tablet_pad_destroy(struct sway_tablet_pad *tablet_pad)
{
    if (!tablet_pad) {
        return;
    }
...
void
sway_tablet_pad_set_focus(struct sway_tablet_pad *tablet_pad,
                          struct wlr_surface *surface)
{
    ...
    if (surface == NULL || !wlr_surface_accepts_tablet_v2(
                               surface, tablet_pad->tablet->tablet_v2)) {
        return;
    }
