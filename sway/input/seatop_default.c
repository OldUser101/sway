 ...
 */
static bool
edge_is_external(struct sway_container *cont, enum wlr_edges edge)
{
...
find_edge(struct sway_container *cont, struct wlr_surface *surface,
 ...
 */
enum wlr_edges
find_resize_edge(struct sway_container *cont, struct wlr_surface *surface,
                 struct sway_cursor *cursor)
{
 ...
 */
static struct sway_binding *
get_active_mouse_binding(struct seatop_default_event *e, list_t *bindings,
                         uint32_t modifiers, bool release, bool on_titlebar,
                         bool on_border, bool on_content, bool on_workspace,
...
static void
handle_tablet_tool_tip(struct sway_seat *seat, struct sway_tablet_tool *tool,
                       uint32_t time_msec, enum wlr_tablet_tool_tip_state state)
{
    ...
    struct sway_node *node = node_at_coords(
        seat, cursor->cursor->x, cursor->cursor->y, &surface, &sx, &sy);

    if (!sway_assert(surface, "Expected null-surface tablet input to route "
                              "through pointer emulation")) {
        return;
    }
    ...
    if ((layer = wlr_layer_surface_v1_try_from_wlr_surface(surface)) &&
        ...
        bool mod_pressed = keyboard && (wlr_keyboard_get_modifiers(keyboard) &
                                        config->floating_mod);

        // Handle beginning floating move
        if (is_floating_or_child && !is_fullscreen_or_child && mod_pressed) {
            seat_set_focus_container(
                seat, seat_get_focus_inactive_view(seat, &cont->node));
...
static bool
trigger_pointer_button_binding(struct sway_seat *seat,
                               struct wlr_input_device *device, uint32_t button,
                               enum wl_pointer_button_state state,
                               uint32_t modifiers, bool on_titlebar,
                               bool on_border, bool on_contents,
                               bool on_workspace)
{
...
static void
handle_button(struct sway_seat *seat, uint32_t time_msec,
              struct wlr_input_device *device, uint32_t button,
              enum wl_pointer_button_state state)
{
    struct sway_cursor *cursor = seat->cursor;
    ...
    enum wlr_edges resize_edge =
        cont && edge ? find_resize_edge(cont, surface, cursor) : WLR_EDGE_NONE;
    ...
    if (cont && resize_edge && button == BTN_LEFT &&
        ...
        if (layout == L_TABBED || layout == L_STACKED) {
            cont_to_focus =
                seat_get_focus_inactive_view(seat, &cont->pending.parent->node);
        }
    ...
    if (cont && !is_floating_or_child && mod_pressed &&
        mod_resize_btn_pressed && state == WL_POINTER_BUTTON_STATE_PRESSED) {
        edge = 0;
    ...
    if (cont && is_floating_or_child && !is_fullscreen_or_child &&
        ...
        if (mod_resize_btn_pressed) {
            ...
            edge |= cursor->cursor->x >
                            floater->pending.x + floater->pending.width / 2
                        ? WLR_EDGE_RIGHT
                        : WLR_EDGE_LEFT;
            edge |= cursor->cursor->y >
                            floater->pending.y + floater->pending.height / 2
                        ? WLR_EDGE_BOTTOM
                        : WLR_EDGE_TOP;
...
static void
check_focus_follows_mouse(struct sway_seat *seat,
                          struct seatop_default_event *e,
                          struct sway_node *hovered_node)
{
...
static void
handle_pointer_motion(struct sway_seat *seat, uint32_t time_msec)
{
    struct seatop_default_event *e = seat->seatop_data;
    struct sway_cursor *cursor = seat->cursor;
...
static void
handle_tablet_tool_motion(struct sway_seat *seat, struct sway_tablet_tool *tool,
                          uint32_t time_msec)
{
    struct seatop_default_event *e = seat->seatop_data;
    struct sway_cursor *cursor = seat->cursor;
    ...
    if (surface) {
        if (seat_is_input_allowed(seat, surface)) {
            ...
            wlr_tablet_v2_tablet_tool_notify_motion(tool->tablet_v2_tool, sx,
                                                    sy);
        }
    } else {
        cursor_update_image(cursor, node);
...
static void
handle_touch_down(struct sway_seat *seat, struct wlr_touch_down_event *event,
                  double lx, double ly)
{
    struct wlr_surface *surface = NULL;
...
static void
handle_pointer_axis(struct sway_seat *seat,
                    struct wlr_pointer_axis_event *event)
{
    ...
    bool on_titlebar_border =
        cont && on_border && cursor->cursor->y < cont->pending.content_y;
    bool on_contents = cont && !on_border && surface;
    bool on_workspace = node && node->type == N_WORKSPACE;
    float scroll_factor =
 ...
 */
static bool
gesture_binding_check(list_t *bindings, enum gesture_type type, uint8_t fingers,
 ...
 */
static struct sway_gesture_binding *
gesture_binding_match(list_t *bindings, struct gesture *gesture,
                      const char *input)
{
...
static struct sway_gesture_binding *
gesture_tracker_end_and_match(struct gesture_tracker *tracker,
                              struct sway_input_device *device)
{
...
static void
gesture_binding_execute(struct sway_seat *seat,
...
static void
handle_hold_begin(struct sway_seat *seat,
                  struct wlr_pointer_hold_begin_event *event)
{
    ...
    if (gesture_binding_check(bindings, GESTURE_TYPE_HOLD, event->fingers,
                              device)) {
        struct seatop_default_event *seatop = seat->seatop_data;
        gesture_tracker_begin(&seatop->gestures, GESTURE_TYPE_HOLD,
                              event->fingers);
    } else {
...
static void
handle_hold_end(struct sway_seat *seat,
                struct wlr_pointer_hold_end_event *event)
{
    ...
    struct sway_input_device *device =
        event->pointer ? event->pointer->base.data : NULL;
    struct sway_gesture_binding *binding =
        gesture_tracker_end_and_match(&seatop->gestures, device);
...
static void
handle_pinch_begin(struct sway_seat *seat,
                   struct wlr_pointer_pinch_begin_event *event)
{
    ...
    struct sway_input_device *device =
        event->pointer ? event->pointer->base.data : NULL;
    list_t *bindings = config->current_mode->gesture_bindings;
    if (gesture_binding_check(bindings, GESTURE_TYPE_PINCH, event->fingers,
                              device)) {
        struct seatop_default_event *seatop = seat->seatop_data;
        gesture_tracker_begin(&seatop->gestures, GESTURE_TYPE_PINCH,
                              event->fingers);
    } else {
        // ... otherwise forward to client
        struct sway_cursor *cursor = seat->cursor;
...
static void
handle_pinch_update(struct sway_seat *seat,
...
static void
handle_pinch_end(struct sway_seat *seat,
...
static void
handle_swipe_begin(struct sway_seat *seat,
                   struct wlr_pointer_swipe_begin_event *event)
{
    ...
    struct sway_input_device *device =
        event->pointer ? event->pointer->base.data : NULL;
    list_t *bindings = config->current_mode->gesture_bindings;
    if (gesture_binding_check(bindings, GESTURE_TYPE_SWIPE, event->fingers,
                              device)) {
        struct seatop_default_event *seatop = seat->seatop_data;
        gesture_tracker_begin(&seatop->gestures, GESTURE_TYPE_SWIPE,
                              event->fingers);
    } else {
        // ... otherwise forward to client
        struct sway_cursor *cursor = seat->cursor;
...
static void
handle_swipe_update(struct sway_seat *seat,
                    struct wlr_pointer_swipe_update_event *event)
{
    ...
    if (gesture_tracker_check(&seatop->gestures, GESTURE_TYPE_SWIPE)) {
        gesture_tracker_update(&seatop->gestures, event->dx, event->dy, NAN,
                               NAN);
    } else {
        // ... otherwise forward to client
        struct sway_cursor *cursor = seat->cursor;
...
static void
handle_swipe_end(struct sway_seat *seat,
...
static void
handle_rebase(struct sway_seat *seat, uint32_t time_msec)
{
    struct seatop_default_event *e = seat->seatop_data;
    struct sway_cursor *cursor = seat->cursor;
    struct wlr_surface *surface = NULL;
    double sx = 0.0, sy = 0.0;
    e->previous_node = node_at_coords(seat, cursor->cursor->x,
...
void
seatop_begin_default(struct sway_seat *seat)
{
    seatop_end(seat);
