 ...
 */
struct sway_node *
node_at_coords(struct sway_seat *seat, double lx, double ly,
               struct wlr_surface **surface, double *sx, double *sy)
{
    ...
    wl_list_for_each_reverse(node, &root->layer_tree->children, link)
    {
        ...
        bool non_interactive = scene_descriptor_try_get(
            &layer->node, SWAY_SCENE_DESC_NON_INTERACTIVE);
        if (non_interactive) {
            continue;
        }
    ...
    if (scene_node) {
        ...
        while (true) {
            struct sway_container *con =
                scene_descriptor_try_get(current, SWAY_SCENE_DESC_CONTAINER);

            if (!con) {
                struct sway_view *view =
                    scene_descriptor_try_get(current, SWAY_SCENE_DESC_VIEW);
                if (view) {
                    con = view->container;
                }
            }
            ...
            if (scene_descriptor_try_get(current,
...
void
cursor_rebase(struct sway_cursor *cursor)
{
...
void
cursor_update_image(struct sway_cursor *cursor, struct sway_node *node)
{
    if (node && node->type == N_CONTAINER) {
        // Try a node's resize edge
        enum wlr_edges edge =
            find_resize_edge(node->sway_container, NULL, cursor);
...
static void
cursor_hide(struct sway_cursor *cursor)
{
...
int
cursor_get_timeout(struct sway_cursor *cursor)
{
...
void
cursor_notify_key_press(struct sway_cursor *cursor)
{
    if (cursor->hidden) {
        return;
    }
...
static enum sway_input_idle_source
...
void
cursor_handle_activity_from_idle_source(struct sway_cursor *cursor,
                                        enum sway_input_idle_source idle_source)
{
    wl_event_source_timer_update(cursor->hide_source,
                                 cursor_get_timeout(cursor));
...
void
cursor_handle_activity_from_device(struct sway_cursor *cursor,
...
void
cursor_unhide(struct sway_cursor *cursor)
{
    if (!cursor->hidden) {
        return;
    }
...
void
pointer_motion(struct sway_cursor *cursor, uint32_t time_msec,
...
handle_pointer_button(struct wl_listener *listener, void *data)
{
    ...
    cursor_handle_activity_from_device(cursor, &event->pointer->base);
    dispatch_cursor_button(cursor, &event->pointer->base, event->time_msec,
                           event->button, event->state);
}

void
dispatch_cursor_axis(struct sway_cursor *cursor,
...
static void
handle_touch_cancel(struct wl_listener *listener, void *data)
{
    struct sway_cursor *cursor =
        wl_container_of(listener, cursor, touch_cancel);
...
handle_touch_motion(struct wl_listener *listener, void *data)
{
    ...
    if (cursor->simulating_pointer_from_touch) {
        if (seat->touch_id == cursor->pointer_touch_id) {
            ...
            pointer_motion(cursor, event->time_msec, &event->touch->base, dx,
                           dy, dx, dy);
        }
    } else {
...
{
    struct sway_cursor *cursor = wl_container_of(listener, cursor, touch_frame);
...
static void
apply_mapping_from_region(struct wlr_input_device *device,
                          struct input_config_mapped_from_region *region,
                          double *x, double *y)
{
...
static void
handle_tablet_tool_position(struct sway_cursor *cursor,
                            struct sway_tablet_tool *tool, bool change_x,
                            bool change_y, double x, double y, double dx,
                            double dy, int32_t time_msec)
{
    ...
    struct wlr_surface *surface = NULL;
    struct sway_seat *seat = cursor->seat;
    node_at_coords(seat, cursor->cursor->x, cursor->cursor->y, &surface, &sx,
                   &sy);
    ...
    if (!cursor->simulating_pointer_from_tool_tip &&
        ...
        pointer_motion(cursor, time_msec, input_device->wlr_device, dx, dy, dx,
                       dy);
    }
}
...
handle_tool_axis(struct wl_listener *listener, void *data)
{
    ...
    handle_tablet_tool_position(
        cursor, sway_tool, event->updated_axes & WLR_TABLET_TOOL_AXIS_X,
        event->updated_axes & WLR_TABLET_TOOL_AXIS_Y, event->x, event->y,
        event->dx, event->dy, event->time_msec);

    if (event->updated_axes & WLR_TABLET_TOOL_AXIS_PRESSURE) {
        wlr_tablet_v2_tablet_tool_notify_pressure(sway_tool->tablet_v2_tool,
                                                  event->pressure);
    }

    if (event->updated_axes & WLR_TABLET_TOOL_AXIS_DISTANCE) {
        wlr_tablet_v2_tablet_tool_notify_distance(sway_tool->tablet_v2_tool,
                                                  event->distance);
    }
    ...
    if (event->updated_axes &
        ...
        wlr_tablet_v2_tablet_tool_notify_tilt(
            sway_tool->tablet_v2_tool, sway_tool->tilt_x, sway_tool->tilt_y);
    }

    if (event->updated_axes & WLR_TABLET_TOOL_AXIS_ROTATION) {
        wlr_tablet_v2_tablet_tool_notify_rotation(sway_tool->tablet_v2_tool,
                                                  event->rotation);
    }

    if (event->updated_axes & WLR_TABLET_TOOL_AXIS_SLIDER) {
        wlr_tablet_v2_tablet_tool_notify_slider(sway_tool->tablet_v2_tool,
                                                event->slider);
    }

    if (event->updated_axes & WLR_TABLET_TOOL_AXIS_WHEEL) {
        wlr_tablet_v2_tablet_tool_notify_wheel(sway_tool->tablet_v2_tool,
                                               event->wheel_delta, 0);
    }
}
...
static struct sway_tablet *
get_tablet_for_device(struct sway_cursor *cursor,
                      struct wlr_input_device *device)
{
...
handle_tool_proximity(struct wl_listener *listener, void *data)
{
    ...
    handle_tablet_tool_position(cursor, sway_tool, true, true, event->x,
                                event->y, 0, 0, event->time_msec);
}
...
handle_tool_button(struct wl_listener *listener, void *data)
{
    ...
    node_at_coords(cursor->seat, cursor->cursor->x, cursor->cursor->y, &surface,
                   &sx, &sy);
    ...
    bool should_simulate_pointer =
        mod_pressed || !surface_supports_tablet_events;
    ...
    if ((cursor->tool_buttons > 0 &&
         cursor->simulating_pointer_from_tool_button) ||
        ...
        wlr_tablet_v2_tablet_tool_notify_button(
            sway_tool->tablet_v2_tool, event->button,
...
static void
check_constraint_region(struct sway_cursor *cursor)
{
    ...
    if (cursor->active_confine_requires_warp && view) {
        ...
        double sx =
            cursor->cursor->x - con->pending.content_x + view->geometry.x;
        double sy =
            cursor->cursor->y - con->pending.content_y + view->geometry.y;
...
{
    struct sway_cursor *cursor = wl_container_of(listener, cursor, hold_begin);
...
{
    struct sway_cursor *cursor = wl_container_of(listener, cursor, hold_end);
...
{
    struct sway_cursor *cursor = wl_container_of(listener, cursor, pinch_begin);
...
{
    struct sway_cursor *cursor = wl_container_of(listener, cursor, pinch_end);
...
{
    struct sway_cursor *cursor = wl_container_of(listener, cursor, swipe_begin);
...
{
    struct sway_cursor *cursor = wl_container_of(listener, cursor, swipe_end);
...
static void
set_image_surface(struct sway_cursor *cursor, struct wlr_surface *surface)
{
...
void
cursor_set_image(struct sway_cursor *cursor, const char *image,
...
void
cursor_set_image_surface(struct sway_cursor *cursor,
                         struct wlr_surface *surface, int32_t hotspot_x,
...
void
sway_cursor_destroy(struct sway_cursor *cursor)
{
    if (!cursor) {
        return;
    }
...
struct sway_cursor *
sway_cursor_create(struct sway_seat *seat)
{
 ...
 */
void
cursor_warp_to_container(struct sway_cursor *cursor,
 ...
 */
void
cursor_warp_to_workspace(struct sway_cursor *cursor,
...
get_mouse_bindcode(const char *name, char **error)
{
    ...
    if (!event || !has_prefix(event, "BTN_")) {
        *error = format_str("Event code %d (%s) is not a button", code,
                            event ? event : "(null)");
        return 0;
    }
    return code;
...
static void
warp_to_constraint_cursor_hint(struct sway_cursor *cursor)
{
    struct wlr_pointer_constraint_v1 *constraint = cursor->active_constraint;
...
handle_request_set_cursor_shape(struct wl_listener *listener, void *data)
{
    ...
    cursor_set_image(seat->cursor, wlr_cursor_shape_v1_name(event->shape),
                     focused_client);
}
