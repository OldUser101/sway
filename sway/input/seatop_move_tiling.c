...
static void
handle_end(struct sway_seat *seat)
{
...
static void
handle_motion_prethreshold(struct sway_seat *seat)
{
    struct seatop_move_tiling_event *e = seat->seatop_data;
...
static void
split_border(double pos, int offset, int len, int n_children, int avoid,
             int *out_pos, bool *out_after)
{
    ...
    *out_after = region % 2;
    ...
    if (child_index == 0) {
        ...
        *out_pos =
            offset + child_index * len / n_children - DROP_SPLIT_INDICATOR / 2;
    }
}

static bool
split_titlebar(struct sway_node *node, struct sway_container *avoid,
               struct wlr_cursor *cursor, struct wlr_box *title_box,
               bool *after)
{
    ...
    if (layout == L_STACKED && cursor->y < box.y + title_height * n_children) {
        ...
        title_box->x = box.x;
        split_border(cursor->y, box.y, title_height * n_children, n_children,
        ...
        title_box->y = box.y;
        split_border(cursor->x, box.x, box.width, n_children, avoid_index,
                     &title_box->x, after);
        return true;
    }
    return false;
...
static void
update_indicator(struct seatop_move_tiling_event *e, struct wlr_box *box)
{
...
static void
handle_motion_postthreshold(struct sway_seat *seat)
{
    struct seatop_move_tiling_event *e = seat->seatop_data;
    ...
    if (!surface && !container_has_ancestor(con, e->con) &&
        split_titlebar(node, e->con, cursor->cursor, &drop_box,
    ...
    int thresh_bottom = con->pending.content_y + con->pending.content_height -
                        DROP_LAYOUT_BORDER;
    int thresh_left = con->pending.content_x + DROP_LAYOUT_BORDER;
    int thresh_right = con->pending.content_x + con->pending.content_width -
                       DROP_LAYOUT_BORDER;
    while (con) {
    ...
    size_t thickness =
        fmin(con->pending.content_width, con->pending.content_height) * 0.3;
    ...
    if ((dist = con->pending.x + con->pending.width - cursor->cursor->x) <
        closest_dist) {
        closest_dist = dist;
        e->target_edge = WLR_EDGE_RIGHT;
    }
    if ((dist = con->pending.y + con->pending.height - cursor->cursor->y) <
        closest_dist) {
        closest_dist = dist;
...
static bool
is_parallel(enum sway_container_layout layout, enum wlr_edges edge)
{
...
static void
finalize_move(struct sway_seat *seat)
{
    struct seatop_move_tiling_event *e = seat->seatop_data;
    ...
    if (target_node->type == N_WORKSPACE && edge == WLR_EDGE_NONE) {
        ...
        if (swap) {
            ...
            if (edge && !is_parallel(layout, edge)) {
                enum sway_container_layout new_layout =
                    edge == WLR_EDGE_TOP || edge == WLR_EDGE_BOTTOM ? L_VERT
                                                                    : L_HORIZ;
        ...
        enum sway_container_layout new_layout =
            edge == WLR_EDGE_TOP || edge == WLR_EDGE_BOTTOM ? L_VERT : L_HORIZ;
...
static void
handle_unref(struct sway_seat *seat, struct sway_container *con)
{
    struct seatop_move_tiling_event *e = seat->seatop_data;
...
void
seatop_begin_move_tiling_threshold(struct sway_seat *seat,
                                   struct sway_container *con)
{
    seatop_end(seat);
...
void
seatop_begin_move_tiling(struct sway_seat *seat, struct sway_container *con)
{
