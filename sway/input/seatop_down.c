...
static void
handle_touch_motion(struct sway_seat *seat,
...
static void
handle_touch_up(struct sway_seat *seat, struct wlr_touch_up_event *event)
{
    struct seatop_down_event *e = seat->seatop_data;
                  ...
                  double lx, double ly)
{
    ...
    struct sway_node *focused_node =
        node_at_coords(seat, seat->touch_x, seat->touch_y, &surface, &sx, &sy);

    if (!surface ||
        surface != e->surface) { // Must start from the initial surface
        return;
    }
...
static void
handle_touch_cancel(struct sway_seat *seat,
...
void
seatop_begin_down(struct sway_seat *seat, struct sway_container *con, double sx,
                  double sy)
{
...
void
seatop_begin_touch_down(struct sway_seat *seat, struct wlr_surface *surface,
                        struct wlr_touch_down_event *event, double sx,
                        double sy, double lx, double ly)
{
...
void
seatop_begin_down_on_surface(struct sway_seat *seat,
