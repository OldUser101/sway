...
static void
seat_device_destroy(struct sway_seat_device *seat_device)
{
    if (!seat_device) {
        return;
    }
...
static void
seat_node_destroy(struct sway_seat_node *seat_node)
{
...
void
seat_destroy(struct sway_seat *seat)
{
...
void
seat_idle_notify_activity(struct sway_seat *seat,
                          enum sway_input_idle_source source)
{
    if ((source & seat->idle_inhibit_sources) == 0) {
        return;
    }
    wlr_idle_notifier_v1_notify_activity(server.idle_notifier_v1,
                                         seat->wlr_seat);
}
 ...
 */
static void
seat_send_activate(struct sway_node *node, struct sway_seat *seat)
{
...
static struct sway_keyboard *
sway_keyboard_for_wlr_keyboard(struct sway_seat *seat,
...
static void
seat_keyboard_notify_enter(struct sway_seat *seat, struct wlr_surface *surface)
{
...
static void
seat_tablet_pads_set_focus(struct sway_seat *seat, struct wlr_surface *surface)
{
    struct sway_seat_device *seat_device;
    wl_list_for_each(seat_device, &seat->devices, link)
    {
 ...
 */
static void
seat_send_focus(struct sway_node *node, struct sway_seat *seat)
{
    seat_send_activate(node, seat);

    struct sway_view *view =
        node->type == N_CONTAINER ? node->sway_container->view : NULL;
...
void
seat_for_each_node(struct sway_seat *seat,
...
seat_get_focus_inactive_view(struct sway_seat *seat, struct sway_node *ancestor)
{
...
static void
handle_seat_node_destroy(struct wl_listener *listener, void *data)
{
    ...
    while (next_focus == NULL && parent != NULL) {
        struct sway_container *con = seat_get_focus_inactive_view(seat, parent);
    ...
    if (needs_new_focus) {
        ...
        if (focus->type == N_CONTAINER &&
            ...
            seat_set_raw_focus(seat,
                               &focus->sway_container->pending.workspace->node);
        }
        seat_set_raw_focus(seat, focus);
    }
}

static struct sway_seat_node *
seat_node_from_node(struct sway_seat *seat, struct sway_node *node)
{
...
static void
handle_new_node(struct wl_listener *listener, void *data)
{
...
static void
drag_icon_update_position(struct sway_seat *seat, struct wlr_scene_node *node)
{
    struct wlr_drag_icon *wlr_icon =
        scene_descriptor_try_get(node, SWAY_SCENE_DESC_DRAG_ICON);
    ...
    switch (wlr_icon->drag->grab_type) {
        ...
        case WLR_DRAG_GRAB_KEYBOARD_TOUCH:;
            struct wlr_touch_point *point = wlr_seat_touch_get_point(
                seat->wlr_seat, wlr_icon->drag->touch_id);
            if (point == NULL) {
                return;
            }
...
void
drag_icons_update_position(struct sway_seat *seat)
{
...
handle_start_drag(struct wl_listener *listener, void *data)
{
    ...
    if (wlr_drag_icon != NULL) {
        struct wlr_scene_tree *tree =
            wlr_scene_drag_icon_create(seat->drag_icons, wlr_drag_icon);
        if (!tree) {
        ...
        if (!scene_descriptor_assign(&tree->node, SWAY_SCENE_DESC_DRAG_ICON,
                                     wlr_drag_icon)) {
            sway_log(SWAY_ERROR,
                     "Failed to allocate a drag icon scene descriptor");
...
static void
collect_focus_iter(struct sway_node *node, void *data)
{
...
static void
collect_focus_workspace_iter(struct sway_workspace *workspace, void *data)
{
    collect_focus_iter(&workspace->node, data);
}

static void
collect_focus_container_iter(struct sway_container *container, void *data)
{
...
static void
seat_update_capabilities(struct sway_seat *seat)
{
    uint32_t caps = 0;
...
static void
seat_reset_input_config(struct sway_seat *seat,
...
static bool
is_touch_or_tablet_tool(struct sway_seat_device *seat_device)
{
    switch (seat_device->input_device->wlr_device->type) {
        case WLR_INPUT_DEVICE_TOUCH:
        case WLR_INPUT_DEVICE_TABLET:
...
static void
seat_apply_input_mapping(struct sway_seat *seat,
                         struct sway_seat_device *sway_device)
{
    ...
    switch (mapped_to) {
        case MAPPED_TO_DEFAULT:;
            ...
            switch (dev->type) {
                case WLR_INPUT_DEVICE_POINTER:
                    mapped_to_output =
                        wlr_pointer_from_input_device(dev)->output_name;
                    break;
                case WLR_INPUT_DEVICE_TOUCH:
                    mapped_to_output =
                        wlr_touch_from_input_device(dev)->output_name;
                    break;
                default:
        ...
        case MAPPED_TO_OUTPUT:
            ...
            if (strcmp("*", mapped_to_output) == 0) {
                wlr_cursor_map_input_to_output(
                    seat->cursor->cursor, sway_device->input_device->wlr_device,
                    NULL);
            ...
            wlr_cursor_map_input_to_region(
                seat->cursor->cursor, sway_device->input_device->wlr_device,
                NULL);
            sway_log(SWAY_DEBUG, "Mapped to output %s",
                     output->wlr_output->name);
            return;
        case MAPPED_TO_REGION:
            sway_log(SWAY_DEBUG, "Mapping input device %s to %d,%d %dx%d",
                     sway_device->input_device->identifier, mapped_to_region->x,
...
static void
seat_configure_pointer(struct sway_seat *seat,
                       struct sway_seat_device *sway_device)
{
...
static void
seat_configure_keyboard(struct sway_seat *seat,
                        struct sway_seat_device *seat_device)
{
...
static void
seat_configure_switch(struct sway_seat *seat,
                      struct sway_seat_device *seat_device)
{
...
static void
seat_configure_touch(struct sway_seat *seat,
                     struct sway_seat_device *sway_device)
{
    wlr_cursor_attach_input_device(seat->cursor->cursor,
                                   sway_device->input_device->wlr_device);
...
static void
seat_configure_tablet_tool(struct sway_seat *seat,
                           struct sway_seat_device *sway_device)
{
...
static void
seat_configure_tablet_pad(struct sway_seat *seat,
                          struct sway_seat_device *sway_device)
{
...
static struct sway_seat_device *
seat_get_device(struct sway_seat *seat, struct sway_input_device *input_device)
{
...
void
seat_configure_device(struct sway_seat *seat,
                      struct sway_input_device *input_device)
{
...
void
seat_configure_device_mapping(struct sway_seat *seat,
                              struct sway_input_device *input_device)
{
    struct sway_seat_device *seat_device = seat_get_device(seat, input_device);
    if (!seat_device) {
...
void
seat_reset_device(struct sway_seat *seat,
                  struct sway_input_device *input_device)
{
    struct sway_seat_device *seat_device = seat_get_device(seat, input_device);
    if (!seat_device) {
...
void
seat_add_device(struct sway_seat *seat, struct sway_input_device *input_device)
{
...
void
seat_remove_device(struct sway_seat *seat,
                   struct sway_input_device *input_device)
{
    struct sway_seat_device *seat_device = seat_get_device(seat, input_device);
...
static bool
xcursor_manager_is_named(const struct wlr_xcursor_manager *manager,
                         const char *name)
{
...
void
seat_configure_xcursor(struct sway_seat *seat)
{
    ...
    if (seat == input_manager_get_default_seat()) {
        ...
        if (server.xwayland.wlr_xwayland &&
            ...
            if (xcursor != NULL) {
                ...
                wlr_xwayland_set_cursor(server.xwayland.wlr_xwayland, buffer,
    ...
    if (!seat->cursor->xcursor_manager ||
        !xcursor_manager_is_named(seat->cursor->xcursor_manager,
                                  cursor_theme) ||
        ...
        seat->cursor->xcursor_manager =
            wlr_xcursor_manager_create(cursor_theme, cursor_size);
        if (!seat->cursor->xcursor_manager) {
            sway_log(SWAY_ERROR, "Cannot create XCursor manager for theme '%s'",
                     cursor_theme);
        }
...
bool
seat_is_input_allowed(struct sway_seat *seat, struct wlr_surface *surface)
{
...
static void
send_unfocus(struct sway_container *con, void *data)
{
    if (con->view) {
...
static void
seat_send_unfocus(struct sway_node *node, struct sway_seat *seat)
{
...
static void
set_workspace(struct sway_seat *seat, struct sway_workspace *new_ws)
{
    ...
    if (seat->workspace) {
        seat->prev_workspace = seat->workspace->num;
    }
...
void
seat_set_raw_focus(struct sway_seat *seat, struct sway_node *node)
{
    struct sway_seat_node *seat_node = seat_node_from_node(seat, node);
    wl_list_remove(&seat_node->link);
    wl_list_insert(&seat->focus_stack, &seat_node->link);
...
static void
seat_set_workspace_focus(struct sway_seat *seat, struct sway_node *node)
{
    ...
    struct sway_workspace *new_workspace =
        node->type == N_WORKSPACE ? node->sway_workspace
                                  : node->sway_container->pending.workspace;
    struct sway_container *container =
        node->type == N_CONTAINER ? node->sway_container : NULL;

    // Deny setting focus to a view which is hidden by a fullscreen container or
    // global
...
void
seat_set_focus(struct sway_seat *seat, struct sway_node *node)
{
...
void
seat_set_focus_container(struct sway_seat *seat, struct sway_container *con)
{
    seat_set_focus(seat, con ? &con->node : NULL);
}

void
seat_set_focus_workspace(struct sway_seat *seat, struct sway_workspace *ws)
{
    seat_set_focus(seat, ws ? &ws->node : NULL);
}

void
seat_set_focus_surface(struct sway_seat *seat, struct wlr_surface *surface,
                       bool unfocus)
{
...
void
seat_set_focus_layer(struct sway_seat *seat, struct wlr_layer_surface_v1 *layer)
{
...
void
seat_unfocus_unless_client(struct sway_seat *seat, struct wl_client *client)
{
    ...
    wl_list_for_each(point, &seat->wlr_seat->touch_state.touch_points, link)
    {
        if (point->client->client != client) {
            wlr_seat_touch_point_clear_focus(seat->wlr_seat, now.tv_nsec / 1000,
                                             point->touch_id);
        }
    }
}
...
struct sway_node *
seat_get_focus_inactive(struct sway_seat *seat, struct sway_node *node)
{
    if (node_is_view(node)) {
        return node;
    }
...
struct sway_container *
seat_get_focus_inactive_tiling(struct sway_seat *seat,
...
struct sway_node *
seat_get_active_tiling_child(struct sway_seat *seat, struct sway_node *parent)
{
...
seat_get_focused_workspace(struct sway_seat *seat)
{
...
struct sway_workspace *
seat_get_last_known_workspace(struct sway_seat *seat)
{
    struct sway_seat_node *current;
    wl_list_for_each(current, &seat->focus_stack, link)
    {
...
struct sway_container *
seat_get_focused_container(struct sway_seat *seat)
{
    struct sway_node *focus = seat_get_focus(seat);
...
void
seat_apply_config(struct sway_seat *seat, struct seat_config *seat_config)
{
    struct sway_seat_device *seat_device = NULL;
...
struct seat_config *
seat_get_config(struct sway_seat *seat)
{
...
void
seat_pointer_notify_button(struct sway_seat *seat, uint32_t time_msec,
...
void
seat_consider_warp_to_focus(struct sway_seat *seat)
{
    struct sway_node *focus = seat_get_focus(seat);
    ...
    if (config->mouse_warping == WARP_OUTPUT) {
        ...
        if (output) {
            ...
            if (wlr_box_contains_point(&box, seat->cursor->cursor->x,
                                       seat->cursor->cursor->y)) {
                return;
            }
        }
...
void
seatop_unref(struct sway_seat *seat, struct sway_container *con)
{
...
void
seatop_button(struct sway_seat *seat, uint32_t time_msec,
...
void
seatop_pointer_motion(struct sway_seat *seat, uint32_t time_msec)
{
...
void
seatop_pointer_axis(struct sway_seat *seat,
...
void
seatop_touch_motion(struct sway_seat *seat,
...
void
seatop_touch_up(struct sway_seat *seat, struct wlr_touch_up_event *event)
{
...
void
seatop_touch_down(struct sway_seat *seat, struct wlr_touch_down_event *event,
                  double lx, double ly)
{
...
void
seatop_touch_cancel(struct sway_seat *seat,
...
void
seatop_tablet_tool_tip(struct sway_seat *seat, struct sway_tablet_tool *tool,
...
void
seatop_tablet_tool_motion(struct sway_seat *seat, struct sway_tablet_tool *tool,
                          uint32_t time_msec)
{
...
void
seatop_hold_begin(struct sway_seat *seat,
...
void
seatop_hold_end(struct sway_seat *seat,
...
void
seatop_pinch_begin(struct sway_seat *seat,
...
void
seatop_pinch_update(struct sway_seat *seat,
...
void
seatop_pinch_end(struct sway_seat *seat,
...
void
seatop_swipe_begin(struct sway_seat *seat,
...
void
seatop_swipe_update(struct sway_seat *seat,
...
void
seatop_swipe_end(struct sway_seat *seat,
...
void
seatop_end(struct sway_seat *seat)
{
...
bool
seatop_allows_set_cursor(struct sway_seat *seat)
{
