...
static bool
set_rotation_angle(struct libinput_device *device, double angle)
{
...
static bool
set_clickfinger_button_map(struct libinput_device *device,
                           enum libinput_config_clickfinger_button_map map)
{
    if (libinput_device_config_click_get_clickfinger_button_map(device) ==
        map) {
        return false;
    }
...
static bool
set_scroll_button_lock(struct libinput_device *dev,
...
static bool
set_dwtp(struct libinput_device *device, bool dwtp)
{
...
static bool
configure_send_events(struct libinput_device *device, struct input_config *ic)
{
...
bool
sway_input_configure_libinput_device(struct sway_input_device *input_device)
{
    ...
    if (ic->drag_lock != INT_MIN) {
        ...
        changed |=
            set_tap_drag_lock(device, LIBINPUT_CONFIG_DRAG_LOCK_ENABLED_STICKY);
#endif
    }
    ...
    if (ic->clickfinger_button_map != INT_MIN) {
        changed |=
            set_clickfinger_button_map(device, ic->clickfinger_button_map);
    }
...
void
sway_input_configure_libinput_device_send_events(
...
void
sway_input_reset_libinput_device(struct sway_input_device *input_device)
{
    ...
    changed |= set_rotation_angle(
        device, libinput_device_config_rotation_get_default_angle(device));
    ...
    changed |= set_clickfinger_button_map(
        device, libinput_device_config_click_get_default_clickfinger_button_map(
                    device));
    ...
    changed |= set_dwtp(
        device, libinput_device_config_dwtp_get_default_enabled(device));
...
static bool
sway_udev_device_is_builtin(struct udev_device *udev_device)
{
    const char *id_path =
...
bool
sway_libinput_device_is_builtin(struct sway_input_device *sway_device)
{
