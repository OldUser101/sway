...
struct sway_switch *
sway_switch_create(struct sway_seat *seat, struct sway_seat_device *device)
{
    struct sway_switch *switch_device = calloc(1, sizeof(struct sway_switch));
...
static bool
sway_switch_trigger_test(enum sway_switch_trigger trigger,
...
static void
execute_binding(struct sway_switch *sway_switch)
{
...
void
sway_switch_configure(struct sway_switch *sway_switch)
{
...
void
sway_switch_destroy(struct sway_switch *sway_switch)
{
    if (!sway_switch) {
        return;
    }
...
void
sway_switch_retrigger_bindings_for_all(void)
{
