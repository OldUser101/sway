...
static struct sway_container *
container_get_resize_sibling(struct sway_container *con, uint32_t edge)
{
    if (!con) {
        return NULL;
    }
...
handle_pointer_motion(struct sway_seat *seat, uint32_t time_msec)
{
    ...
    if (e->h_con) {
        if (e->edge & WLR_EDGE_LEFT) {
            amount_x =
                (e->h_con_orig_width - moved_x) - e->h_con->pending.width;
    ...
    if (e->v_con) {
        if (e->edge & WLR_EDGE_TOP) {
            amount_y =
                (e->v_con_orig_height - moved_y) - e->v_con->pending.height;
...
void
seatop_begin_resize_tiling(struct sway_seat *seat, struct sway_container *con,
                           enum wlr_edges edge)
{
    seatop_end(seat);
