...
struct sway_seat *
input_manager_sway_seat_from_wlr_seat(struct wlr_seat *wlr_seat)
{
    struct sway_seat *seat = NULL;
...
static bool
device_is_touchpad(struct sway_input_device *device)
{
#if WLR_HAS_LIBINPUT_BACKEND
...
const char *
input_device_get_type(struct sway_input_device *device)
{
...
static void
apply_input_type_config(struct sway_input_device *input_device)
{
...
input_sway_device_from_wlr(struct wlr_input_device *device)
{
...
handle_new_input(struct wl_listener *listener, void *data)
{
    ...
    sway_log(SWAY_DEBUG, "adding device: '%s'", input_device->identifier);
    ...
    wl_list_for_each(seat, &input->seats, link)
    {
        struct seat_config *seat_config = seat_get_config(seat);
        bool has_attachment =
            seat_config && (seat_config_get_attachment(
    ...
    if (!added) {
        sway_log(SWAY_DEBUG, "device '%s' is not configured on any seats",
...
handle_virtual_keyboard(struct wl_listener *listener, void *data)
{
    ...
    struct sway_seat *seat =
        keyboard->seat ? input_manager_sway_seat_from_wlr_seat(keyboard->seat)
...
static void
handle_transient_seat_manager_create_seat(struct wl_listener *listener,
                                          void *data)
{
...
input_manager_create(struct sway_server *server)
{
    struct sway_input_manager *input =
...
void
input_manager_finish(struct sway_input_manager *input)
{
...
bool
input_manager_has_focus(struct sway_node *node)
{
    struct sway_seat *seat = NULL;
    wl_list_for_each(seat, &server.input->seats, link)
    {
...
void
input_manager_set_focus(struct sway_node *node)
{
    struct sway_seat *seat;
    wl_list_for_each(seat, &server.input->seats, link)
    {
...
static void
input_manager_configure_input(struct sway_input_device *input_device)
{
#if WLR_HAS_LIBINPUT_BACKEND
    bool config_changed = sway_input_configure_libinput_device(input_device);
#else
...
void
input_manager_configure_all_input_mappings(void)
{
    struct sway_input_device *input_device;
    wl_list_for_each(input_device, &server.input->devices, link)
    {
...
input_manager_apply_input_config(struct input_config *input_config)
{
    ...
    {
        bool type_matches =
            type_wildcard && strcmp(input_device_get_type(input_device),
                                    input_config->identifier + 5) == 0;
...
void
input_manager_reset_input(struct sway_input_device *input_device)
{
#if WLR_HAS_LIBINPUT_BACKEND
...
struct input_config *
input_device_get_config(struct sway_input_device *device)
{
