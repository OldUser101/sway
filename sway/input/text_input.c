...
static struct sway_text_input *
relay_get_focusable_text_input(struct sway_input_method_relay *relay)
{
...
static struct sway_text_input *
relay_get_focused_text_input(struct sway_input_method_relay *relay)
{
    struct sway_text_input *text_input = NULL;
    wl_list_for_each(text_input, &relay->text_inputs, link)
    {
...
static void
text_input_set_pending_focused_surface(struct sway_text_input *text_input,
...
static void
constrain_popup(struct sway_input_popup *popup)
{
    ...
    wlr_scene_node_coords(&popup->desc.relative->parent->node, &parent.x,
                          &parent.y);
    ...
    wlr_scene_node_set_position(popup->desc.relative, x - parent.x - geo.x,
                                y - parent.y - geo.y);
    if (cursor_rect) {
        struct wlr_box box = {
            .x = x1 - x,
    ...
    if (popup->scene_tree) {
        wlr_scene_node_set_position(&popup->scene_tree->node, x - geo.x,
                                    y - geo.y);
    }
}
...
static void
relay_send_im_state(struct sway_input_method_relay *relay,
...
static void
relay_disable_text_input(struct sway_input_method_relay *relay,
                         struct sway_text_input *text_input)
{
...
sway_text_input_create(struct sway_input_method_relay *relay,
...
static void
input_popup_set_focus(struct sway_input_popup *popup,
                      struct wlr_surface *surface)
{
    ...
    if (layer_surface) {
        ...
        // In the future there may be other shells been added, so we also need
        // to check here.
        if (view == NULL) {
...
handle_im_popup_surface_unmap(struct wl_listener *listener, void *data)
{
    ...
    // relative should already be freed as it should be a child of the just
    // unmapped scene
    popup->desc.relative = NULL;
...
static void
text_input_send_enter(struct sway_text_input *text_input,
                      struct wlr_surface *surface)
{
...
static void
sway_input_method_relay_finish_text_input(struct sway_input_method_relay *relay)
{
...
static void
relay_handle_text_input_manager_destroy(struct wl_listener *listener,
                                        void *data)
{
    struct sway_input_method_relay *relay =
        wl_container_of(listener, relay, text_input_manager_destroy);

    sway_input_method_relay_finish_text_input(relay);
}

static void
sway_input_method_relay_finish_input_method(
    struct sway_input_method_relay *relay)
{
...
static void
relay_handle_input_method_manager_destroy(struct wl_listener *listener,
                                          void *data)
{
    struct sway_input_method_relay *relay =
        wl_container_of(listener, relay, input_method_manager_destroy);

    sway_input_method_relay_finish_input_method(relay);
}

void
sway_input_method_relay_init(struct sway_seat *seat,
                             struct sway_input_method_relay *relay)
{
    relay->seat = seat;
    ...
    relay->text_input_manager_destroy.notify =
        relay_handle_text_input_manager_destroy;
    ...
    wl_signal_add(&server.input_method->events.new_input_method,
                  &relay->input_method_new);
    relay->input_method_manager_destroy.notify =
        relay_handle_input_method_manager_destroy;
    wl_signal_add(&server.input_method->events.destroy,
                  &relay->input_method_manager_destroy);
}

void
sway_input_method_relay_finish(struct sway_input_method_relay *relay)
{
    sway_input_method_relay_finish_text_input(relay);
    sway_input_method_relay_finish_input_method(relay);
}
...
void
sway_input_method_relay_set_focus(struct sway_input_method_relay *relay,
                                  struct wlr_surface *surface)
{
    struct sway_text_input *text_input;
    wl_list_for_each(text_input, &relay->text_inputs, link)
