...
bool
criteria_is_empty(struct criteria *criteria)
{
    return !criteria->title && !criteria->shell && !criteria->all &&
           !criteria->app_id && !criteria->con_mark && !criteria->con_id
#if WLR_HAS_XWAYLAND
           && !criteria->class && !criteria->id && !criteria->instance &&
           !criteria->window_role && criteria->window_type == ATOM_LAST
#endif
           && !criteria->floating && !criteria->tiling && !criteria->urgent &&
           !criteria->workspace && !criteria->pid &&
           !criteria->sandbox_engine && !criteria->sandbox_app_id &&
...
static bool
generate_regex(pcre2_code **regex, char *value)
{
    int errorcode;
    PCRE2_SIZE offset;
...
static bool
pattern_create(struct pattern **pattern, char *value)
{
...
static void
pattern_destroy(struct pattern *pattern)
{
    if (pattern) {
        if (pattern->regex) {
...
void
criteria_destroy(struct criteria *criteria)
{
...
static int
regex_cmp(const char *item, const pcre2_code *regex)
{
    pcre2_match_data *match_data =
        pcre2_match_data_create_from_pattern(regex, NULL);
    int result = pcre2_match(regex, (PCRE2_SPTR)item, strlen(item), 0, 0,
                             match_data, NULL);
...
#if WLR_HAS_XWAYLAND
static bool
view_has_window_type(struct sway_view *view, enum atom_name name)
{
...
static int
cmp_urgent(const void *_a, const void *_b)
{
...
static bool
has_container_criteria(struct criteria *criteria)
{
    return criteria->con_mark || criteria->con_id;
}

static bool
criteria_matches_container(struct criteria *criteria,
                           struct sway_container *container)
{
    if (criteria->con_mark) {
        ...
        for (int i = 0; i < con->marks->length; ++i) {
            if (regex_cmp(con->marks->items[i], criteria->con_mark->regex) >=
                0) {
                exists = true;
                break;
            }
...
static bool
criteria_matches_view(struct criteria *criteria, struct sway_view *view)
{
    ...
    if (criteria->app_id) {
        ...
        switch (criteria->app_id->match_type) {
            case PATTERN_FOCUSED:
                if (focused &&
                    lenient_strcmp(app_id, view_get_app_id(focused))) {
                    return false;
                }
                break;
            case PATTERN_PCRE2:
    ...
    if (criteria->sandbox_engine) {
        ...
        switch (criteria->sandbox_engine->match_type) {
            case PATTERN_FOCUSED:
                if (focused &&
                    lenient_strcmp(sandbox_engine,
                                   view_get_sandbox_engine(focused))) {
                    return false;
                }
                break;
            case PATTERN_PCRE2:
                if (regex_cmp(sandbox_engine, criteria->sandbox_engine->regex) <
                    0) {
                    return false;
                }
                break;
    ...
    if (criteria->sandbox_app_id) {
        ...
        switch (criteria->sandbox_app_id->match_type) {
            case PATTERN_FOCUSED:
                if (focused &&
                    lenient_strcmp(sandbox_app_id,
                                   view_get_sandbox_app_id(focused))) {
                    return false;
                }
                break;
            case PATTERN_PCRE2:
                if (regex_cmp(sandbox_app_id, criteria->sandbox_app_id->regex) <
                    0) {
                    return false;
                }
                break;
    ...
    if (criteria->sandbox_instance_id) {
        ...
        switch (criteria->sandbox_instance_id->match_type) {
            case PATTERN_FOCUSED:
                if (focused &&
                    lenient_strcmp(sandbox_instance_id,
                                   view_get_sandbox_instance_id(focused))) {
                    return false;
                }
                break;
            case PATTERN_PCRE2:
                if (regex_cmp(sandbox_instance_id,
                              criteria->sandbox_instance_id->regex) < 0) {
                    return false;
                }
                break;
        }
    ...
    if (criteria->instance) {
        ...
        switch (criteria->instance->match_type) {
            case PATTERN_FOCUSED:
                if (focused &&
                    lenient_strcmp(instance, view_get_instance(focused))) {
                    return false;
                }
                break;
            case PATTERN_PCRE2:
    ...
    if (criteria->window_role) {
        ...
        switch (criteria->window_role->match_type) {
            case PATTERN_FOCUSED:
                if (focused && lenient_strcmp(window_role,
                                              view_get_window_role(focused))) {
                    return false;
                }
                break;
            case PATTERN_PCRE2:
    ...
    if (criteria->workspace) {
        ...
        switch (criteria->workspace->match_type) {
            case PATTERN_FOCUSED:
                if (focused &&
                    ws->num == focused->container->pending.workspace->num) {
                    return false;
                }
                break;
            case PATTERN_PCRE2:
                // not supported with workspace numbers
                return false;
        }
    }
...
list_t *
criteria_for_view(struct sway_view *view, enum criteria_type types)
{
...
static void
criteria_get_containers_iterator(struct sway_container *container, void *data)
{
...
list_t *
criteria_get_containers(struct criteria *criteria)
{
    list_t *matches = create_list();
...
static enum criteria_token
token_from_name(char *name)
{
...
static bool
parse_token(struct criteria *criteria, char *name, char *value)
{
    ...
    switch (token) {
        ...
        case T_CON_ID:
            if (strcmp(value, "__focused__") == 0) {
                ...
                if (*endptr != 0) {
                    error = strdup("The value for 'con_id' should be "
                                   "'__focused__' or numeric");
                }
            }
            break;
        case T_CON_MARK:
        ...
        case T_TILING:
            ...
            break;
        case T_URGENT:
            if (strcmp(value, "latest") == 0 || strcmp(value, "newest") == 0 ||
                strcmp(value, "last") == 0 || strcmp(value, "recent") == 0) {
                criteria->urgent = 'l';
 ...
 */
struct criteria *
criteria_parse(char *raw, char **error_arg)
{
    *error_arg = NULL;
    error = NULL;
...
bool
criteria_is_equal(struct criteria *left, struct criteria *right)
{
...
bool
criteria_already_exists(struct criteria *criteria)
{
