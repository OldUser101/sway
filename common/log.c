...
bool
_sway_assert(bool condition, const char *format, ...)
{
    if (condition) {
        return true;
    }
...
static void
sway_log_stderr(sway_log_importance_t verbosity, const char *fmt, va_list args)
{
    init_start_time();
    ...
    unsigned c = (verbosity < SWAY_LOG_IMPORTANCE_LAST)
                     ? verbosity
...
void
sway_log_init(sway_log_importance_t verbosity, terminate_callback_t callback)
{
    init_start_time();
...
void
_sway_vlog(sway_log_importance_t verbosity, const char *fmt, va_list args)
{
    sway_log_stderr(verbosity, fmt, args);
}

void
_sway_log(sway_log_importance_t verbosity, const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    sway_log_stderr(verbosity, fmt, args);
