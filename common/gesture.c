...
char *
gesture_parse(const char *input, struct gesture *output)
{
    ...
    if (split->length < 1 || split->length > 3) {
        return format_str("expected <gesture>[:<fingers>][:direction], got %s",
                          input);
    }
...
const char *
gesture_type_string(enum gesture_type type)
{
    switch (type) {
...
const char *
gesture_direction_string(enum gesture_direction direction)
{
    switch (direction) {
...
static char *
gesture_directions_to_string(uint32_t directions)
{
    char *result = NULL;
...
char *
gesture_to_string(struct gesture *gesture)
{
    char *directions = gesture_directions_to_string(gesture->directions);
    char *result = format_str("%s:%u:%s", gesture_type_string(gesture->type),
...
bool
gesture_check(struct gesture *target, enum gesture_type type, uint8_t fingers)
{
...
bool
gesture_match(struct gesture *target, struct gesture *to_match, bool exact)
{
...
bool
gesture_equal(struct gesture *a, struct gesture *b)
{
...
static uint8_t
gesture_directions_count(uint32_t directions)
{
    uint8_t count = 0;
...
static int8_t
gesture_directions_compare(uint32_t a, uint32_t b)
{
...
int8_t
gesture_compare(struct gesture *a, struct gesture *b)
{
    return gesture_directions_compare(a->directions, b->directions);
}

void
gesture_tracker_begin(struct gesture_tracker *tracker, enum gesture_type type,
                      uint8_t fingers)
{
    tracker->type = type;
...
bool
gesture_tracker_check(struct gesture_tracker *tracker, enum gesture_type type)
{
    return tracker->type == type;
}

void
gesture_tracker_update(struct gesture_tracker *tracker, double dx, double dy,
                       double scale, double rotation)
{
    ...
    sway_log(SWAY_DEBUG, "update tracking %s:%u:? gesture: %f %f %f %f",
             gesture_type_string(tracker->type), tracker->fingers, tracker->dx,
             tracker->dy, tracker->scale, tracker->rotation);
}

void
gesture_tracker_cancel(struct gesture_tracker *tracker)
{
...
struct gesture *
gesture_tracker_end(struct gesture_tracker *tracker)
{
