...
void sway_log_init(sway_log_importance_t verbosity,
                   terminate_callback_t terminate);

void _sway_log(sway_log_importance_t verbosity, const char *format, ...)
    ATTRIB_PRINTF(2, 3);
void _sway_vlog(sway_log_importance_t verbosity, const char *format,
                va_list args) ATTRIB_PRINTF(2, 0);
...
#define sway_assert(COND, FMT, ...)                                            \
    _sway_assert(COND, "[%s:%d] %s:" FMT, _SWAY_FILENAME, __LINE__,            \
