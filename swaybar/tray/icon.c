...
get_basedirs(void)
{
    ...
    list_add(basedirs,
             strdup(data_home && *data_home ? "$XDG_DATA_HOME/icons"
...
find_icon_in_subdir(char *name, char *basedir, char *theme, char *subdir)
{
    ...
    for (size_t i = 0; i < sizeof(extensions) / sizeof(*extensions); ++i) {
        char *path = format_str("%s/%s/%s/%s.%s", basedir, theme, subdir, name,
                                extensions[i]);
...
static char *
find_icon_with_theme(list_t *basedirs, list_t *themes, char *name, int size,
                     char *theme_name, int *min_size, int *max_size)
{
    ...
    if (!icon && theme->inherits) {
        for (int i = 0; i < theme->inherits->length; ++i) {
            icon = find_icon_with_theme(basedirs, themes, name, size,
                                        theme->inherits->items[i], min_size,
                                        max_size);
            if (icon) {
                break;
            }
...
char *
find_icon(list_t *themes, list_t *basedirs, char *name, int size, char *theme,
