...
static void
add_sni(struct swaybar_tray *tray, char *id)
{
...
handle_sni_unregistered(sd_bus_message *msg, void *data, sd_bus_error *error)
{
    ...
    int ret = sd_bus_message_read(msg, "s", &id);
    if (ret < 0) {
        sway_log(SWAY_ERROR, "Failed to parse unregister SNI message: %s",
                 strerror(-ret));
    }
...
static int
get_registered_snis_callback(sd_bus_message *msg, void *data,
                             sd_bus_error *error)
{
    ...
    if (ret < 0) {
        sway_log(SWAY_ERROR, "Failed to read registered SNIs: %s",
                 strerror(-ret));
        return ret;
    }
...
static bool
register_to_watcher(struct swaybar_host *host)
{
    ...
    int ret = sd_bus_call_method_async(
        ...
        host->service);
    if (ret < 0) {
        sway_log(SWAY_ERROR, "Failed to send register call: %s",
                 strerror(-ret));
        return false;
    }
    ...
    if (ret < 0) {
        sway_log(SWAY_ERROR, "Failed to get registered SNIs: %s",
                 strerror(-ret));
    }
...
bool
init_host(struct swaybar_host *host, char *protocol, struct swaybar_tray *tray)
{
    host->watcher_interface =
        format_str("org.%s.StatusNotifierWatcher", protocol);
    ...
    ret = sd_bus_match_signal(tray->bus, &watcher_slot, "org.freedesktop.DBus",
                              ...
                              "NameOwnerChanged", handle_new_watcher, host);
    if (ret < 0) {
        sway_log(SWAY_ERROR, "Failed to subscribe to unregistering events: %s",
                 strerror(-ret));
        goto error;
...
void
finish_host(struct swaybar_host *host)
{
