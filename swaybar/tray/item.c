...
static bool
sni_ready(struct swaybar_sni *sni)
{
    return sni->status &&
...
static void
set_sni_dirty(struct swaybar_sni *sni)
{
    if (sni_ready(sni)) {
...
static int
read_pixmap(sd_bus_message *msg, struct swaybar_sni *sni, const char *prop,
            list_t **dest)
{
    ...
    while (!sd_bus_message_at_end(msg, 0)) {
        ...
        if (height > 0 && width == height) {
            sway_log(SWAY_DEBUG, "%s %s: found icon w:%d h:%d", sni->watcher_id,
                     prop, width, height);
...
static void
sni_get_property_async(struct swaybar_sni *sni, const char *prop,
 ...
 */
static int
sni_check_msg_sender(struct swaybar_sni *sni, sd_bus_message *msg,
                     const char *signal)
{
...
handle_new_icon(sd_bus_message *msg, void *data, sd_bus_error *error)
{
    ...
    if (!strcmp(sni->interface, "org.kde.StatusNotifierItem")) {
        sni_get_property_async(sni, "IconThemePath", "s",
                               &sni->icon_theme_path);
    }
...
handle_new_status(sd_bus_message *msg, void *data, sd_bus_error *error)
{
    ...
    if (ret == 1) {
        ...
        if (r < 0) {
            ...
            sway_log(SWAY_DEBUG, "%s has new status = '%s'", sni->watcher_id,
                     status);
            set_sni_dirty(sni);
        }
    } else {
...
static void
sni_match_signal_async(struct swaybar_sni *sni, char *signal,
                       sd_bus_message_handler_t callback)
{
    ...
    int ret = sd_bus_match_signal_async(sni->tray->bus, &slot->slot,
                                        sni->service, sni->path, sni->interface,
...
struct swaybar_sni *
create_sni(char *id, struct swaybar_tray *tray)
{
...
void
destroy_sni(struct swaybar_sni *sni)
{
    if (!sni) {
        return;
    }
...
static void
handle_click(struct swaybar_sni *sni, int x, int y, uint32_t button, int delta)
{
    ...
    if (has_prefix(method, "Scroll")) {
        ...
        sd_bus_call_method_async(sni->tray->bus, NULL, sni->service, sni->path,
                                 sni->interface, method, NULL, NULL, "ii", x,
                                 y);
    }
}
...
static void
reload_sni(struct swaybar_sni *sni, char *icon_theme, int target_size)
{
    char *icon_name =
    ...
    if (icon_name) {
        ...
        char *icon_path =
            find_icon(sni->tray->themes, icon_search_paths, icon_name,
                      target_size, icon_theme, &sni->min_size, &sni->max_size);
...
uint32_t
render_sni(cairo_t *cairo, struct swaybar_output *output, double *x,
           struct swaybar_sni *sni)
{
    ...
    if (sni->icon) {
        ...
        icon = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, icon_size,
                                          icon_size);
