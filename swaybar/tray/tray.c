...
struct swaybar_tray *
create_tray(struct swaybar *bar)
{
    ...
    if (ret < 0) {
        sway_log(SWAY_ERROR, "Failed to connect to user bus: %s",
                 strerror(-ret));
        return NULL;
    }
...
void
destroy_tray(struct swaybar_tray *tray)
{
    if (!tray) {
        return;
    }
...
uint32_t
render_tray(cairo_t *cairo, struct swaybar_output *output, double *x)
{
