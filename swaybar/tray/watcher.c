...
static bool
using_standard_protocol(struct swaybar_watcher *watcher)
{
...
handle_lost_service(sd_bus_message *msg, void *data, sd_bus_error *error)
{
    ...
    if (ret < 0) {
        sway_log(SWAY_ERROR, "Failed to parse owner change message: %s",
                 strerror(-ret));
        return ret;
    }
    ...
    if (!*new_owner) {
        ...
        for (int idx = 0; idx < watcher->items->length; ++idx) {
            ...
            bool cmp_res = using_standard_protocol(watcher)
                               ? cmp_id(id, service) == 0
                               : has_prefix(id, service);
            if (cmp_res) {
                sway_log(SWAY_DEBUG, "Unregistering Status Notifier Item '%s'",
                         id);
        ...
        if (idx != -1) {
            sway_log(SWAY_DEBUG, "Unregistering Status Notifier Host '%s'",
                     service);
...
register_sni(sd_bus_message *msg, void *data, sd_bus_error *error)
{
    ...
    if (ret < 0) {
        sway_log(SWAY_ERROR, "Failed to parse register SNI message: %s",
                 strerror(-ret));
        return ret;
    }
    ...
    if (list_seq_find(watcher->items, cmp_id, id) == -1) {
        ...
        sway_log(SWAY_DEBUG, "Status Notifier Item '%s' already registered",
                 id);
        free(id);
    }
...
register_host(sd_bus_message *msg, void *data, sd_bus_error *error)
{
    ...
    if (ret < 0) {
        sway_log(SWAY_ERROR, "Failed to parse register host message: %s",
                 strerror(-ret));
        return ret;
    }
    ...
    if (list_seq_find(watcher->hosts, cmp_id, service) == -1) {
        ...
        sway_log(SWAY_DEBUG, "Status Notifier Host '%s' already registered",
                 service);
    }
...
static int
get_registered_snis(sd_bus *bus, const char *obj_path, const char *interface,
...
static int
is_host_registered(sd_bus *bus, const char *obj_path, const char *interface,
                   const char *property, sd_bus_message *reply, void *data,
                   sd_bus_error *error)
{
    struct swaybar_watcher *watcher = data;
...
create_watcher(char *protocol, sd_bus *bus)
{
    ...
    if (ret < 0) {
        if (-ret == EEXIST) {
            sway_log(SWAY_DEBUG,
                     "Failed to acquire service name '%s':"
...
void
destroy_watcher(struct swaybar_watcher *watcher)
{
    if (!watcher) {
        return;
    }
