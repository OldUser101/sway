...
void
update_cursor(struct swaybar_seat *seat)
{
...
static bool
check_bindings(struct swaybar *bar, uint32_t button, uint32_t state)
{
...
static bool
process_hotspots(struct swaybar_output *output, double x, double y,
...
static void
workspace_next(struct swaybar *bar, struct swaybar_output *output, bool left)
{
...
static void
process_discrete_scroll(struct swaybar_seat *seat,
                        struct swaybar_output *output,
                        struct swaybar_pointer *pointer, uint32_t axis,
                        wl_fixed_t value)
{
    uint32_t button = wl_axis_to_button(axis, value);
    if (process_hotspots(output, pointer->x, pointer->y, button,
                         WL_POINTER_BUTTON_STATE_PRESSED)) {
        // (Currently hotspots don't do anything on release events, so no need
        // to emit one)
        return;
    }

    // If there is a button press binding, execute it, and check button release
    // bindings
    ...
    if (!sway_assert(!wl_list_empty(&output->workspaces),
                     "axis with no workspaces")) {
        return;
    }
...
static void
process_continuous_scroll(struct swaybar_seat *seat,
                          struct swaybar_output *output,
                          struct swaybar_pointer *pointer, uint32_t axis)
{
...
get_touch_slot(struct swaybar_touch *touch, int32_t id)
{
    ssize_t next = -1;
...
void
swaybar_seat_free(struct swaybar_seat *seat)
{
    if (!seat) {
        return;
    }
