...
i3bar_parse_json_color(json_object *json, uint32_t *color)
{
    ...
    if (!color_set) {
        sway_log(SWAY_ERROR,
                 "Ignoring invalid block hexadecimal color string: %s",
                 hexstring);
    }
    return color_set;
}
...
i3bar_parse_json(struct status_line *status, struct json_object *json_array)
{
    ...
    for (size_t i = 0; i < json_object_array_length(json_array); ++i) {
        ...
        json_object_object_get_ex(json, "separator_block_width",
        ...
        block->separator_block_width =
            separator_block_width ? json_object_get_int(separator_block_width)
                                  : 9;
        // Airblader features
    ...
    while (!status->started) { // look for opening bracket
        for (size_t c = 0; c < status->buffer_index; ++c) {
            if (status->buffer[c] == '[') {
                ...
                memmove(status->buffer, &status->buffer[c],
                        status->buffer_index);
                break;
    ...
    while (true) {
        ...
        if (status->expecting_comma) {
            ...
            if (err == json_tokener_success) {
                ...
                // full)
                char *last_char_pos =
                    &status->buffer[buffer_pos + status->tokener->char_offset -
                                    1];
                ...
                *last_char_pos = last_char;
                ...
                sway_log(SWAY_DEBUG, "Failed to parse i3bar json - %s: '%s%c'",
                         ...
                         &status->buffer[buffer_pos], last_char);
        ...
        errno = 0;
        ssize_t read_bytes =
            read(status->read_fd, &status->buffer[status->buffer_index],
...
i3bar_block_send_click(struct status_line *status, struct i3bar_block *block,
                       double x, double y, double rx, double ry, double w,
                       double h, int scale, uint32_t button, bool released)
{
    ...
    if (status->float_event_coords) {
        ...
        json_object_object_add(event_json, "relative_x",
                               json_object_new_double(rx));
        json_object_object_add(event_json, "relative_y",
                               json_object_new_double(ry));
        ...
        json_object_object_add(event_json, "relative_x",
                               json_object_new_int(rx));
        json_object_object_add(event_json, "relative_y",
                               json_object_new_int(ry));
