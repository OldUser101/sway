...
render_status_line_error(struct render_context *ctx, double *x)
{
    ...
    get_text_size(cairo, font, &text_width, &text_height, NULL, 1, false, "%s",
                  error);
...
render_status_line_text(struct render_context *ctx, double *x)
{
    ...
    get_text_size(cairo, config->font_description, &text_width, &text_height,
                  NULL, 1, config->pango_markup, "%s", text);
    ...
    *x -= text_width + margin;
    ...
    render_text(cairo, config->font_description, 1, config->pango_markup, "%s",
                text);
    *x -= margin;
    return output->height;
}
...
static void
render_sharp_rectangle(cairo_t *cairo, uint32_t color, double x, double y,
...
static void
render_sharp_line(cairo_t *cairo, uint32_t color, double x, double y,
                  double width, double height)
{
    if (width > 1 && height > 1) {
...
block_hotspot_callback(struct swaybar_output *output,
...
static uint32_t
render_status_block(struct render_context *ctx, struct i3bar_block *block,
                    double *x, bool edge, bool use_short_text)
{
    ...
    if (block->min_width_str) {
        int w;
        get_text_size(cairo, config->font_description, &w, NULL, NULL, 1,
                      block->markup, "%s", block->min_width_str);
        block->min_width = w;
    }
    ...
    *x -= width;
    ...
    if (!edge) {
        if (config->sep_symbol) {
            get_text_size(cairo, config->font_description, &sep_width,
                          &sep_height, NULL, 1, false, "%s",
                          config->sep_symbol);
    ...
    if (output->bar->status->click_events) {
        struct swaybar_hotspot *hotspot =
            calloc(1, sizeof(struct swaybar_hotspot));
        hotspot->x = *x;
        hotspot->y = 0;
        hotspot->width = width;
    ...
    if (bg_color) {
        render_sharp_rectangle(cairo, bg_color, x_pos, y_pos, block_width,
                               render_height);
    ...
    if (block->border_set || block->urgent) {
        if (block->border_top > 0) {
            render_sharp_line(cairo, border_color, x_pos, y_pos, block_width,
                              block->border_top);
        }
    ...
    if (!edge && block->separator) {
        ...
        if (config->sep_symbol) {
            ...
            choose_text_aa_mode(ctx, color);
            render_text(cairo, config->font_description, 1, false, "%s",
                        config->sep_symbol);
        } else {
            cairo_set_operator(cairo, CAIRO_OPERATOR_SOURCE);
...
static void
predict_status_block_pos(cairo_t *cairo, struct swaybar_output *output,
                         struct i3bar_block *block, double *x, bool edge)
{
    if (!block->full_text || !*block->full_text) {
        return;
    }
    ...
    int text_width, text_height;
    get_text_size(cairo, config->font_description, &text_width, &text_height,
                  NULL, 1, block->markup, "%s", block->full_text);
...
static double
predict_status_line_pos(cairo_t *cairo, struct swaybar_output *output, double x)
{
...
static uint32_t
predict_workspace_button_length(cairo_t *cairo, struct swaybar_output *output,
                                struct swaybar_workspace *ws)
{
    ...
    int text_width, text_height;
    get_text_size(cairo, config->font_description, &text_width, &text_height,
                  NULL, 1, config->pango_markup, "%s", ws->label);
...
static uint32_t
predict_workspace_buttons_length(cairo_t *cairo, struct swaybar_output *output)
{
    uint32_t width = 0;
...
static uint32_t
predict_binding_mode_indicator_length(cairo_t *cairo,
                                      struct swaybar_output *output)
{
    ...
    int text_width, text_height;
    get_text_size(cairo, config->font_description, &text_width, &text_height,
                  NULL, 1, output->bar->mode_pango_markup, "%s", mode);
...
render_binding_mode_indicator(struct render_context *ctx, double x)
{
    ...
    ctx->has_transparency |=
        (config->colors.binding_mode.background & 0xFF) != 0xFF;
    ...
    render_text(cairo, config->font_description, 1,
                output->bar->mode_pango_markup, "%s", mode);
    return output->height;
}

static enum hotspot_event_handling
workspace_hotspot_callback(struct swaybar_output *output,
                           struct swaybar_hotspot *hotspot, double x, double y,
                           uint32_t button, bool released, void *data)
{
    if (button != BTN_LEFT) {
...
static uint32_t
render_workspace_button(struct render_context *ctx,
...
static void
output_frame_handle_done(void *data, struct wl_callback *callback,
                         uint32_t time)
{
...
void
render_frame(struct swaybar_output *output)
{
    ...
    struct render_context ctx = {
        .output = output,
        // initial background color used for deciding the best way to antialias
        // text
    ...
    if (height != output->height || output->width == 0) {
        ...
        wl_callback_add_listener(frame_callback, &output_frame_listener,
                                 output);
