...
void
ipc_send_workspace_command(struct swaybar *bar, const char *ws)
{
...
static void
ipc_parse_colors(struct swaybar_config *config, json_object *colors)
{
    struct {
        const char *name;
        uint32_t *color;
...
static bool
ipc_parse_config(struct swaybar_config *config, const char *payload)
{
    ...
    if (json_object_object_get_ex(bar_config, "success", &success) &&
        ...
        sway_log(SWAY_ERROR,
                 "No bar with that ID. Use 'swaymsg -t "
    ...
    if ((json_object_object_get_ex(bar_config, "tray_outputs",
                                   &tray_outputs))) {
    ...
    if ((json_object_object_get_ex(bar_config, "tray_padding",
                                   &tray_padding))) {
    ...
    if ((json_object_object_get_ex(bar_config, "tray_bindings",
                                   &tray_bindings))) {
...
bool
ipc_get_workspaces(struct swaybar *bar)
{
...
void
ipc_execute_binding(struct swaybar *bar, struct swaybar_binding *bind)
{
    ...
    free(ipc_single_command(bar->ipc_socketfd, IPC_COMMAND, bind->command,
                            &len));
}

bool
ipc_initialize(struct swaybar *bar)
{
    uint32_t len = strlen(bar->id);
    char *res = ipc_single_command(bar->ipc_socketfd, IPC_GET_BAR_CONFIG,
                                   bar->id, &len);
    ...
    char *subscribe = "[ \"barconfig_update\", \"bar_state_update\", \"mode\", "
                      "\"workspace\" ]";
    len = strlen(subscribe);
    free(ipc_single_command(bar->ipc_event_socketfd, IPC_SUBSCRIBE, subscribe,
                            &len));
    return true;
}

static bool
handle_bar_state_update(struct swaybar *bar, json_object *event)
{
    json_object *json_id;
    ...
    json_object_object_get_ex(event, "visible_by_modifier",
                              &visible_by_modifier);
...
static bool
handle_barconfig_update(struct swaybar *bar, const char *payload,
                        json_object *json_config)
{
    ...
    wl_list_for_each_safe(output, tmp_output, &bar->outputs, link)
    {
        ...
        } else if (!pango_font_description_equal(oldcfg->font_description,
                                                 newcfg->font_description)) {
    ...
    if (!bar->status && newcfg->status_command) {
        ...
        bar->status->bar = bar;
        loop_add_fd(bar->eventloop, bar->status->read_fd, POLLIN, status_in,
                    bar);
    }
...
bool
handle_ipc_readable(struct swaybar *bar)
{
    ...
    switch (resp->type) {
        ...
        case IPC_EVENT_MODE: {
            ...
            if (json_object_object_get_ex(result, "change", &json_change)) {
                ...
                free(bar->mode);
                bar->mode =
                    strcmp(change, "default") != 0 ? strdup(change) : NULL;
            ...
            if (json_object_object_get_ex(result, "pango_markup",
                                          &json_pango_markup)) {
                bar->mode_pango_markup =
                    json_object_get_boolean(json_pango_markup);
            }
            break;
        }
