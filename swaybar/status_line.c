    ...
    switch (status->protocol) {
        case PROTOCOL_UNDEF:
            ...
            if (ioctl(status->read_fd, FIONREAD, &available_bytes) == -1) {
                sway_log(SWAY_ERROR,
                         "Unable to read status command output size");
            ...
            if (newline != NULL &&
                ...
                if (json_object_object_get_ex(header, "click_events",
                                              &click_events) &&
                ...
                if (json_object_object_get_ex(header, "float_event_coords",
                                              &float_event_coords) &&
                    json_object_get_boolean(float_event_coords)) {
                    sway_log(SWAY_DEBUG,
                             "Enabling floating-point coordinates.");
        ...
        case PROTOCOL_TEXT:
            while (true) {
                ...
                errno = 0;
                read_bytes = getline(&status->buffer, &status->buffer_size,
                                     status->read);
                if (errno == EAGAIN) {
