...
static void
swaybar_output_free(struct swaybar_output *output)
{
    if (!output) {
        return;
    }
...
static void
set_output_dirty(struct swaybar_output *output)
{
...
static void
add_layer_surface(struct swaybar_output *output)
{
...
void
destroy_layer_surface(struct swaybar_output *output)
{
    if (!output->layer_surface) {
        return;
    }
...
void
set_bar_dirty(struct swaybar *bar)
{
...
bool
determine_bar_visibility(struct swaybar *bar, bool moving_layer)
{
    struct swaybar_config *config = bar->config;
    bool visible =
...
static bool
bar_uses_output(struct swaybar_output *output)
{
...
xdg_output_handle_logical_position(void *data,
                                   struct zxdg_output_v1 *xdg_output, int32_t x,
                                   int32_t y)
{
    struct swaybar_output *output = data;
    output->output_x = x;
...
static void
add_xdg_output(struct swaybar_output *output)
{
    ...
    if (strcmp(interface, wl_compositor_interface.name) == 0) {
        ...
        bar->xdg_output_manager = wl_registry_bind(
            registry, name, &zxdg_output_manager_v1_interface, 2);
               ...
               0) {
        bar->cursor_shape_manager = wl_registry_bind(
...
bool
bar_setup(struct swaybar *bar, const char *socket_path)
{
    bar->visible = true;
...
free_seats(struct wl_list *list)
{
    ...
    wl_list_for_each_safe(seat, tmp, list, link) { swaybar_seat_free(seat); }
}

void
bar_teardown(struct swaybar *bar)
{
#if HAVE_TRAY
