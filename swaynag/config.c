...
swaynag_parse_options(int argc, char **argv, struct swaynag *swaynag,
                      list_t *types, struct swaynag_type *type, char **config,
                      bool *debug)
{
    enum type_options {
    ...
    optind = 1;
    while (1) {
        int c = getopt_long(argc, argv, "b:B:z:Z:c:de:y:f:hlL:m:o:s:t:v", opts,
                            NULL);
        if (c == -1) {
            break;
        }
...
int
swaynag_load_config(char *path, struct swaynag *swaynag, list_t *types)
{
    ...
    while ((nread = getline(&line, &line_size, config)) != -1) {
        ...
        if (line[0] == '[') {
            ...
            *close = '\0';
            ...
            result = swaynag_parse_options(2, argv, swaynag, types, type, NULL,
                                           NULL);
            free(flag);
            if (result != 0) {
                break;
