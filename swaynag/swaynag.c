...
static void
swaynag_button_execute(struct swaynag *swaynag, struct swaynag_button *button)
{
                  ...
                  uint32_t time, uint32_t button, uint32_t state)
{
    ...
    if (swaynag->details.visible &&
        ...
        if (x >= button_up.x && y >= button_up.y &&
            ...
            y < button_up.y + button_up.height && swaynag->details.offset > 0) {
                ...
                uint32_t axis, wl_fixed_t value)
{
    struct swaynag_seat *seat = data;
    struct swaynag *swaynag = seat->swaynag;
    if (!swaynag->details.visible || seat->pointer.x < swaynag->details.x ||
              ...
              const char *interface, uint32_t version)
{
    ...
    if (strcmp(interface, wl_compositor_interface.name) == 0) {
        ...
        struct swaynag_seat *seat = calloc(1, sizeof(struct swaynag_seat));
        if (!seat) {
            perror("calloc");
            return;
        }
...
swaynag_setup_cursors(struct swaynag *swaynag)
{
    ...
    wl_list_for_each(seat, &swaynag->seats, link)
    {
        ...
        p->cursor_surface = wl_compositor_create_surface(swaynag->compositor);
