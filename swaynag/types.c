...
struct swaynag_type *
swaynag_type_new(const char *name)
{
...
void
swaynag_types_add_default(list_t *types)
{
...
struct swaynag_type *
swaynag_type_get(list_t *types, char *name)
{
...
void
swaynag_type_merge(struct swaynag_type *dest, struct swaynag_type *src)
{
    if (!dest || !src) {
        return;
    }
...
void
swaynag_type_free(struct swaynag_type *type)
{
    free(type->name);
...
void
swaynag_types_free(list_t *types)
{
    for (int i = 0; i < types->length; ++i) {
