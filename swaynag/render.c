...
render_message(cairo_t *cairo, struct swaynag *swaynag)
{
    ...
    get_text_size(cairo, swaynag->type->font_description, &text_width,
                  &text_height, NULL, 1, true, "%s", swaynag->message);
    ...
    render_text(cairo, swaynag->type->font_description, 1, false, "%s",
                swaynag->message);
...
static void
render_details_scroll_button(cairo_t *cairo, struct swaynag *swaynag,
                             struct swaynag_button *button)
{
    ...
    render_text(cairo, swaynag->type->font_description, 1, true, "%s",
                button->text);
}

static int
get_detailed_scroll_button_width(cairo_t *cairo, struct swaynag *swaynag)
{
    int up_width, down_width, temp_height;
    get_text_size(cairo, swaynag->type->font_description, &up_width,
                  &temp_height, NULL, 1, true, "%s",
                  swaynag->details.button_up.text);
    get_text_size(cairo, swaynag->type->font_description, &down_width,
                  &temp_height, NULL, 1, true, "%s",
                  swaynag->details.button_down.text);
...
static uint32_t
render_detailed(cairo_t *cairo, struct swaynag *swaynag, uint32_t y)
{
    ...
    PangoLayout *layout =
        get_pango_layout(cairo, swaynag->type->font_description,
                         swaynag->details.message, 1, false);
    pango_layout_set_width(layout, (swaynag->details.width - padding * 2) *
                                       PANGO_SCALE);
    ...
    do {
        ...
        if (ideal_height > SWAYNAG_MAX_HEIGHT) {
            ...
            if (!show_buttons) {
                ...
                pango_layout_set_width(layout,
                                       (swaynag->details.width - padding * 2) *
                                           PANGO_SCALE);
            }
        }
        ...
        pango_layout_set_height(
            layout, (swaynag->details.height - padding * 2) * PANGO_SCALE);
...
static uint32_t
render_button(cairo_t *cairo, struct swaynag *swaynag, int button_index, int *x)
{
...
render_to_cairo(cairo_t *cairo, struct swaynag *swaynag)
{
    ...
    cairo_rectangle(cairo, 0, swaynag->height - border, swaynag->width, border);
    cairo_fill(cairo);
...
render_frame(struct swaynag *swaynag)
{
    ...
    if (height != swaynag->height) {
        ...
        swaynag->current_buffer = get_next_buffer(
            swaynag->shm, swaynag->buffers, swaynag->width * swaynag->scale,
        ...
        wl_surface_attach(swaynag->surface, swaynag->current_buffer->buffer, 0,
                          0);
        wl_surface_damage(swaynag->surface, 0, 0, swaynag->width,
                          swaynag->height);
        wl_surface_commit(swaynag->surface);
        wl_display_roundtrip(swaynag->display);
    }
