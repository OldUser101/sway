...
main(int argc, char **argv)
{
    ...
    bool debug = false;
    status = swaynag_parse_options(argc, argv, NULL, NULL, NULL, &config_path,
                                   &debug);
    if (status != 0) {
        goto cleanup;
    }
    ...
    if (argc > 1) {
        ...
        status = swaynag_parse_options(argc, argv, &swaynag, types, type_args,
                                       NULL, NULL);
        if (status != 0) {
            goto cleanup;
        }
    ...
    if (swaynag.details.message) {
        swaynag.details.button_details =
            calloc(1, sizeof(struct swaynag_button));
        swaynag.details.button_details->text =
            strdup(swaynag.details.details_text);
    ...
    char *font =
        pango_font_description_to_string(swaynag.type->font_description);
