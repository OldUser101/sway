...
{
    json_object *num, *tags, *rect, *visible, *output, *urgent, *layout,
        *representation, *focused;
    json_object_object_get_ex(w, "num", &num);
...
{
    json_object *name, *rect, *focused, *active, *power, *ws, *current_mode,
        *non_desktop;
    ...
    if (json_object_get_boolean(non_desktop)) {
        printf("Output %s '%s %s %s' (non-desktop)\n",
        ...
        printf("Output %s '%s %s %s' (disabled)\n",
               json_object_get_string(name), json_object_get_string(make),
               json_object_get_string(model), json_object_get_string(serial));
    }
        ...
        for (size_t i = 0; i < modes_len; ++i) {
            ...
            if (mode_picture_aspect_ratio &&
                strcmp("none", json_object_get_string(
                                   mode_picture_aspect_ratio)) != 0) {
                printf(" (%s)",
                       json_object_get_string(mode_picture_aspect_ratio));
            }
            printf("\n");
        }
    }
...
static void
pretty_print_dyn_ws(json_object *obj)
{
...
pretty_print_tree(json_object *obj, int indent)
{
    ...
    if (shell != NULL) {
        ...
        const char *foreign_toplevel_id = json_object_get_string(
            json_object_object_get(obj, "foreign_toplevel_identifier"));
        const char *sandbox_engine = json_object_get_string(
            json_object_object_get(obj, "sandbox_engine"));
        const char *sandbox_app_id = json_object_get_string(
            json_object_object_get(obj, "sandbox_app_id"));
        const char *sandbox_instance_id = json_object_get_string(
            json_object_object_get(obj, "sandbox_instance_id"));
    ...
    json_bool floating_nodes =
        json_object_object_get_ex(obj, "floating_nodes", &floating_nodes_obj);
    if (floating_nodes) {
        size_t len = json_object_array_length(floating_nodes_obj);
        for (size_t i = 0; i < len; i++) {
            pretty_print_tree(json_object_array_get_idx(floating_nodes_obj, i),
                              indent + 1);
        }
    }
}
